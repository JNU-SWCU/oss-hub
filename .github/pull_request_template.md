...
Closes #<번호>
<!-- 이슈 완료 조건을 이번 PR이 모두 채웠으면 `Closes #<번호>`.
     증거 있는 PR 범위는 병합 가능하지만 이슈 의무가 남으면 `Refs #<번호>`.
     티켓이 없으면 사용자 승인 범위·완료 조건·비범위를 확인하고 이 줄을 `티켓 없음 — <비어 있지 않은 이유>`로 바꾼다. 있는 이슈를 티켓 없음으로 쓰지 않는다.
     아래 예외 문구는 증거가 실제로 적용되지 않을 때만 쓴다.
     필요한 캡처·다이어그램을 얻지 못한 것은 예외가 아니라 제출 차단 사유다. -->
...
<!-- UI·browser 동작이나 API 응답을 소비하는 화면에 증거가 적용되면 채운다.
     적용 범위는 submit-pr-evidence의 증거 계약을 따른다.
     시각 변경은 요소 행이 먼저고 필수다 — 바뀐 컴포넌트마다 한 행.
     Before/After는 같은 selector로 찍는다. 티켓에 `현재 화면` selector가 있으면 그대로 재사용한다.
     전체 화면 두 장은 표 안이 아니라 표 아래에 링크로 둔다.
     화면 증거가 적용되지 않으면 이 절 본문을 `Before/After 없음 — <이유>`로 바꾼다. -->
<!-- 렌더링 변경 없는 브라우저 동작 변경은 frontend-capture.md의 비시각 변경 계약을 따른다.
     `비시각 브라우저 변경 — <비어 있지 않은 사유>`와 `| 동작 | Before | After |` 표를 쓴다.
     실제 전후 전체 캡처와 브라우저/API 검증 근거가 필요하며, 없는 요소를 만들거나 증거를 면제하지 않는다. -->
...
<!-- 이번에 하지 않은 것 / 리뷰어가 결정해 줘야 하는 것 / 환경 전제는 해당 없으면 "없음"으로 적는다. 체크박스는 넷 다 체크한다.
     디자인 규격은 이번 PR이 쓴 공용 컴포넌트·토큰과 그 규격을 어디서 봤는지(docs/design.md 컴포넌트 카드 이름 또는 Figma 컴포넌트 링크)를 적는다.
     새 색·새 컨트롤을 직접 만들었으면 그 사실과 이유를 적는다. 화면 변경이 없으면 `화면 변경 없음 — <이유>`로 바꾼다. -->

- 이번에 하지 않은 것: <티켓 경계 밖으로 남긴 것과 이유 / 없음>
- 리뷰어가 결정해 줘야 하는 것: <물음 / 없음>
- 환경 전제: <새 env·마이그레이션·수동 절차 / 없음>
- 디자인 규격: <쓴 공용 컴포넌트·토큰과 그 규격 출처(design.md 컴포넌트 카드 이름 또는 Figma 컴포넌트 링크) / 화면 변경 없음 — 이유>
- [ ] 로컬 커밋을 전부 push했다
