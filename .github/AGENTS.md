...
PR에서 실행되는 워크플로와 high-risk 후보 경로 설정. 이 디렉터리의 검사·병합 정책 변경은 ADR-005에 따라 high risk로 판정한다.
...
- `CODEOWNERS`는 `.gitignore`와 동일한 패턴 문법이며 아래쪽 줄이 우선한다. 일반 PR의 사람 상호 리뷰를 강제하지 않고 high-risk 후보를 찾는 데 사용하며, 최종 판정과 이중 accept는 ADR-005를 따른다.
