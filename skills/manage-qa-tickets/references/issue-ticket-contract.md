...
| 옛 속성 | Issue |
| --- | --- |
| `QA 항목` | 제목. 앞으로 쓰는 형식은 `QA<번호>. <명령형 할 일>` |
| `문제 내용` | 본문 `## 문제` |
...
| `증거` | 본문에 첨부하거나 재현 경로로 적는다. 제3자 제품 스크린샷은 넣지 않는다 |
...
다음 `QA<번호>`는 열린 Issue와 닫힌 Issue의 제목에서 가장 큰 `QA<n>` 다음이다.
삭제됐거나 닫힌 번호를 재사용하지 않는다.
문자열 정렬로 최댓값을 자르지 않는다.
`QA99`가 `QA100`보다 뒤에 오므로 `gh issue list` 결과를 숫자로 비교한다.

```bash
gh issue list --repo JNU-SWCU/oss-hub --state all --search "QA in:title" --limit 200 --json number,title,state
```

한 번에 안 끝나면 `--search "QA1 in:title"`, `"QA2 in:title"`처럼 자릿수 범위를 나눠 다시 조회한다.
...
캡처는 본문 앞쪽에 둔다 — `현재 화면`은 기능 결함 티켓에서는 `재현` 바로 뒤, UX·디자인 티켓에서는 `문제` 바로 뒤다.
...
## 현재 화면
<`frontend` 티켓만 쓴다. 문제가 보이는 그 요소를 여기에 둔다.>
