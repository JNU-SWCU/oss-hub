...
- EAI local-part와 Unicode domain은 자동 검사의 허용 예외로 지원하지 않는다. 비ASCII email-shaped token과 punycode IDN 후보는 우회 방지를 위해 보수적으로 차단하며, 실제 제품 입력에서 지원하려면 별도 입력 검증 계약과 합성 fixture를 먼저 추가한다.
