...
두 경로의 env 파일은 값이 다르다. `pnpm dev`는 앱이 호스트에 있으니 호스트명이 전부 `localhost`이고, compose 경로는 서비스 DNS(`postgres`, `object-storage`)를 쓴다. 값을 서로 복사하면 연결이 깨진다.
...
- `pnpm local:verify`는 매 실행 고유한 project name을 쓰고 PostgreSQL·object-storage host port를 공개하지 않아 선행 `pnpm db:up` 스택과 충돌하지 않는다. 다만 ingress 포트(기본 3000)는 콜백 origin에 묶여 고정이라 점유 중이면 preflight에서 실패한다.
