...
| `deploy/nginx/**`, `deploy/host-nginx/**`, `scripts/check-submission-upload-route*.sh` | `probe-nginx-callback-log.sh` 합성 callback 로그 계약 + `check-host-nginx.test.sh` IP TLS·ACME·loopback Compose·POST-only Jenkins 계약 + `check-submission-upload-route.test.sh`/실제 설정의 exact·slash-descendant upload 403 차단 계약 | 실제 OAuth·인증서 값·요청·업로드 없음. 저장소 파일만 읽으므로 **실행 중 컨테이너의 설정이 저장소와 같다는 증거는 아니다** — 그 계약은 [ADR-002](../decisions/ADR-002-CI-CD-파이프라인.md)의 배포 smoke가 증명한다 |
