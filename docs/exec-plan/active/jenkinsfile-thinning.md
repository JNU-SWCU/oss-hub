...
- 어떤 PR이든 head·base ref·base SHA가 바뀌면 `MERGE_READY`를 새 exact head·base에 대해 다시 취득한다(accept 코멘트 게이트는 폐지됐다 — ADR-005 2026-08-04 변경).
...
- 각 P PR은 exact-head `MERGE_READY`가 필요하다 — accept 코멘트(`PM_ACCEPT`·`RISK_ACCEPT`) 게이트는 폐지됐고(ADR-005 2026-08-04 변경), 실제 병합 권한 제한은 GitHub 저장소 설정이 맡는다.
...
- [ ] P1→P2→P4→P5A→P5B 직렬 PR 순서와 각 PR의 HIGH_RISK 분류·MERGE_READY 조건을 기록했다.
