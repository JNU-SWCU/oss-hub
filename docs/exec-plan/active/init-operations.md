...
4. #226 병합 전에는 ADR-005의 full-SHA `MERGE_READY`와 high-risk 이중 accept를 수동 대조하고 admin bypass를 사용하지 않는다.
5. #226 병합 뒤 `merge-policy`를 required status check에 추가하고 stale SHA·잘못된 actor·증거 누락을 fail-closed로 거절하는지 dry-run한다.
...
3. production은 공개 GitHub Release를 배포 후보로 사용한다. HMAC이 검증된 release webhook의 `created`·`published` action만 허용하고, job 입력에는 action과 tag만 전달한다.
4. Jenkins는 `draft=false`, `prerelease=false`, 현재 latest full Release와 일치하는 `vMAJOR.MINOR.PATCH` tag 및 main ancestry를 검증한다.
5. 이어서 #199 댓글에서 @GoBeromsu의 `RELEASE_ACCEPT role=PM`과 @Lumiere001의 `RELEASE_ACCEPT role=TECH_LEAD`가 같은 tag·full SHA에 있는지 확인한다. 누락·actor 불일치·stale SHA는 배포 전에 거절하며, 이 검증이 구현되기 전에는 production job을 활성화하지 않는다.
...
7. 이미 성공한 Release와 같거나 낮은 버전은 영속 상태 파일을 기준으로 성공 no-op 처리한다.
8. webhook secret과 운영 환경 파일은 Jenkins Credentials Store에서만 관리한다. webhook payload, 저장소, Jenkins 로그에 실제 값을 출력하지 않는다.
9. Jenkins 관리자는 공용 계정을 공유하지 않고 개인 계정으로 식별한다. 운영 인계 시 새 담당자의 개인 관리자 계정으로 접속을 확인한 뒤 이전 담당자 권한을 회수하며, 기존 비밀번호를 전달하지 않는다.
...
- 일반 `MERGE_READY`, stale SHA 거절, high-risk 이중 accept 누락 거절 fixture
- main 검증 job과 HMAC 검증 release webhook의 분리, 허용 action·latest full Release 및 동일 tag·SHA 이중 `RELEASE_ACCEPT` 검증 기록
