...
| --- | --- | --- | --- | --- |
| W1 | 관 → 교 | 역할 요청 승인 → STAFF 권한 획득 | 거절 / 회수 후 접근 차단 / 대기 중 접근 차단 | `roles.service.ts` |
...
보호 라우트(`/dashboard` · `/my-repos` · `/settings` · `/staff/dashboard` · `/admin/access`)도 서버가 200 HTML을 반환한다.
