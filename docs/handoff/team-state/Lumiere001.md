...
- 공개 안전성: 비밀값, 실데이터, 개인정보, 내부 호스트, 로컬 경로 없음.
## 2026-09-05 — 압축 파일 거절 사유를 학생이 고칠 수 있는 말로 나눈다

- 상태: review
- Issue: #1108
- PR: (이 PR)
- blocker: 증거 캡처와 작성자 인터뷰 대기 (submit-pr-evidence)
- 내용: 정상 형식인 `.zip`이 압축 **안의 내용** 때문에 막혔는데, 응답 코드가 확장자·서명 실패와 같은 것(SUB_018·MSD_010)이라 학생이 「PDF, HWP, JPG, PNG, ZIP 파일만 제출할 수 있습니다」를 읽었다 — ZIP을 낸 사람에게 ZIP은 낼 수 있다고 답하는 상태였고, 그래서 고칠 곳이 압축 안인데 형식만 다시 손보게 됐다. 검사 함수가 여섯 갈래 이상의 거절 사유를 참/거짓 하나로 접어 돌려주던 것이 원인이라, `isSafeSubmissionZipMetadata`를 `inspectSubmissionZipMetadata`로 바꿔 사유를 돌려주게 했다. 갈래는 「학생이 다음에 할 일이 달라지는가」로만 나눠 여덟이다 — 중첩·비밀번호·압축 방식은 셋 다 「다시 압축」으로 끝나지만 **무엇을 바꿔** 다시 압축해야 하는지가 다르므로 가르고, 항목 단위 용량과 총합 용량은 할 일이 같아 하나로 묶었다(압축률도 같은 이유로 하나). 여기에 읽기 자체가 실패한 경우와 담을 수 없는 항목(링크·이름) 두 갈래가 더 있다. 문구는 `submission-zip-admission.ts` 하나가 소유하고 제출 경로(SUB_025~032)와 서류 경로(MSD_031~038)가 같은 문장·같은 상태(422)를 쓴다 — 코드 체계는 합치지 않았고 대신 두 경로에 같은 압축 파일을 넣어 같은 구분이 나오는지 검사가 지킨다. 상태를 415가 아니라 422로 둔 이유는 형식(`.zip`)은 받는 것이 맞고 막힌 것은 그 안에 담긴 내용이기 때문이다. 화면은 여덟 문장의 사본을 들지 않고 서버 문장을 파일 입력 옆에 세운다(SUB_011과 같은 방식). 막는 기준은 값 하나 바뀌지 않았다.
- 검증: backend lint·typecheck·313 스위트 3,577 테스트, frontend lint·typecheck·336 파일 3,401 테스트, `prettier --write apps/ docs/` 통과. 대조 실험 — frontend 제품 코드 3개만 `origin/main`으로 되돌리면 3개 파일이 실패하고(문구 목록 drift 2건, 체크리스트 화면 1건, 폼 테스트는 새 export가 없어 로드 실패), backend 제품 코드 6개를 되돌리면 새·기존 스위트 4개가 실패한다(옛 함수 이름을 참조하지 못해 컴파일 단계에서 멈추므로 그 안의 120개 테스트가 아예 실행되지 않는다).
- 발견: 합성 자료의 「비밀번호 걸린 항목」이 실제로는 비밀번호 갈래를 한 번도 밟지 않고 있었다. 전통 방식 암호화는 자료 앞에 12바이트를 덧붙이므로 무압축 항목의 압축 크기는 원래 크기 + 12여야 하는데, 옛 합성 자료가 그 12를 빼먹어 읽기 단계의 크기 검사에 먼저 걸렸다. 거절된다는 사실만 보던 때는 드러나지 않다가 갈래를 나누자 보였다. 자료를 실제 형태로 고쳤다.
- 남은 것: 프로그램 작성 업로드(`programs/program-authoring-upload.validation.ts`)도 같은 검사를 쓰지만 교직원 화면이고 이 티켓의 범위(제출·서류)가 아니라 판정을 그대로 뒀다. 갈래별 안내가 필요한지는 별건이다.
- 공개 안전성: 비밀값, 실데이터, 개인정보, 내부 호스트, 로컬 경로 없음. 테스트 자료는 모두 합성이다.
