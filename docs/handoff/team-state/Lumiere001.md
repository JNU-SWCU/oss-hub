...
- PR: (이 PR)
- blocker: 없음
- 내용: `/programs/{id}#activity` 로 들어가도 화면이 맨 위에 머물렀다. 앵커 이동 효과가 마운트 직후 한 번만 도는데, 상세와 그 안의 활동 그래프가 각자 늦게 채워져 그 시점에는 셸의 스크롤 칸이 아직 내용보다 크지 않다. 그래서 `scrollIntoView` 가 조용히 아무 일도 못 하고 끝났다. 레이아웃이 앉을 때까지 짧게(50ms × 40) 다시 맞추고, 사용자가 스스로 스크롤하면 그 자리에서 손을 뗀다 — `scroll` 은 우리가 만든 이동도 똑같이 내보내므로 주도권 판단 근거로 쓰지 않고 wheel·touchstart·keydown·pointerdown 을 본다. 같은 방식이 `admin-access-overlay.tsx:95-136` 에 이미 있어 그 관용구를 따랐다.
- 검증: lint·typecheck 통과, frontend 319 파일·3188 테스트 통과. E2E 4건 통과 — 모바일(390px)·데스크톱(1280px)·해시 없이 진입·사용자 스크롤 후 재당김 없음.
