...
- 공개 안전성: 비밀값, 실데이터, 개인정보, 내부 호스트, 로컬 경로 없음.
## 2026-09-05 — 승인이 되돌려진 학생의 서류 줄에서 「수정」을 걷는다

- 상태: review
- Issue: #1206
- PR: 아직 없음 (작성자 인터뷰 뒤 사람이 연다)
- blocker: 두 건 — ① 걷은 자리에 이유 문장을 **한 번 더** 적을지, 적는다면 어떤 낱말로 할지는 PM이 고를 몫이라 넣지 않았다. 지금은 바로 위 마일스톤 머리줄이 쓰는 기존 문장(「신청 승인 후 제출할 수 있습니다」·「신청 승인 후 제출 상태를 확인할 수 있습니다.」)이 그 이유를 말한다. ② #1098(`fix/1098-milestone-submission-gate`)이 **같은 자리를 반대 방향으로 고친다** — 그쪽은 「수정」을 감추지 않고 흐리게 두고 옆에 「승인 후 제출할 수 있습니다」를 붙인다. 어느 쪽을 남길지와 병합 순서는 사람이 정해야 한다.
- 내용: 교직원이 승인을 되돌리면(`APPROVED` → `SUBMITTED`) 제출 행은 그대로 남아 학생 서류 줄이 계속 「수정」을 내놓지만, 눌러서 내면 서버가 403(MSD_006 「승인된 신청만 제출할 수 있습니다」)으로 거절한다. 화면이 할 수 없는 일을 제안하는 자리다. 잠근 버튼을 남기지 않고 **자리를 비웠다** — 눌리지 않는 항목을 굳이 보여 주면 화면만 복잡해진다는 판단이 같은 작성자에게서 이미 나왔다(#1099 → PR #1200). 되돌려진 상태는 두 값의 **양성 조합**으로만 판정한다: 신청이 지금 `SUBMITTED`·`REJECTED`이고, 그 줄에 이미 낸 제출이 있다. 제출을 만드는 모든 쓰기 경로가 승인을 요구하므로 제출 행의 존재가 「한 번은 승인됐다」는 증거이고, 그 조합에 닿는 길은 되돌리기뿐이다. 「승인이 아니다」라는 부정으로 적지 않은 이유는 값이 흘러오지 않는 화면에서 정상 학생의 「수정」까지 사라지기 때문이다.
- 검증: frontend lint 오류 0(경고 5건은 origin/main에도 있는 `sidebar-drawer.test.tsx` 건)·typecheck 통과, **336 파일 3,386 테스트** 통과. 기준선은 같은 워크트리에서 변경을 stash 하고 직접 재 봤고 335 파일 3,375 테스트였다(+1 파일 +11 테스트 = 이번에 더한 것과 정확히 같다). 대조 실험 3회 — (a) `program-detail-view.tsx`만 origin/main으로 되돌리면 새 화면 시험 1건이 정확히 실패하고 나머지는 통과, (b) 판정 함수를 항상 `false`로 만들면 4건 실패, (c) 서류 줄의 감춤 갈래만 걷으면 3건 실패. 셋 다 원복 후 `git status`로 확인했다.
- 캡처: local-review 하네스(`/local-review/student?to=%2Fprograms%2Fprogram-oss-contest`) 데스크톱 1440x900·모바일 390x844. Before는 제품 코드 3개 파일을 origin/main으로 되돌려 찍고 원복했다. 두 캡처가 **다른 상태**임을 DOM으로 확인했다 — Before는 그 줄에 `{label:'수정', disabled:false}` 버튼이 있고 After는 버튼 배열이 비어 있으며, 그 밖의 DOM은 같다.
- 한계(하네스): 되돌려진 신청 상태는 검토 하네스의 어떤 페르소나로도 도달할 수 없다. 캡처를 위해 `student-journey-fixtures.ts`의 `CONTEST_DETAIL.viewer.applicationStatus`를 임시로 `SUBMITTED`로 바꿔 찍고 원복했다(커밋에 포함하지 않았다). 이 상태를 눈으로 볼 페르소나를 상설로 둘지는 별건이다.
- 범위 밖: 아직 한 번도 내지 않은 줄의 「올리기」는 건드리지 않았다. 되돌려진 학생에게 그 버튼도 서버가 거절하지만, 화면에서 그 상태는 「미신청·승인 대기」와 구별되지 않아 #1098이 신청 판정을 머리줄과 서류 줄에 함께 나눠 주는 방식으로 다루는 자리다. 백엔드 판정은 한 줄도 바꾸지 않았다.
- 주의: `applicationStatus`를 선택 prop으로 두지 않고 **필수**로 만들었다. 이번 작업에 남아 있던 앞선 초안이 정확히 그 함정에 빠져 있었다 — 판정 함수와 prop 은 다 만들어 두고 `program-detail-view.tsx`에서 값을 넘기지 않아, 타입은 통과하는데 화면은 고치기 전과 똑같이 동작했다. 필수로 두면 컴파일러가 호출 자리를 잡는다(테스트 리터럴 12곳이 걸렸다).
- UX 안티패턴 점검(PR 본문에 옮길 것): AP-1 중복 사실 — 통과. 걷은 자리에 문장을 새로 세우지 않아 머리줄과 같은 말을 두 번 하지 않는다. AP-2 겹친 용어 — 통과. 「승인」이 한 뜻(신청 승인)으로만 쓰인다. AP-3 단위 혼용 — 해당 없음. AP-4 상태 과잉표시 — 통과. 배지는 서류 판정 하나이고 신청 상태는 머리줄 한 줄이다. AP-5 액션의 문장화 — 통과. 이번 변경은 컨트롤을 더하지 않고 덜어낸다. AP-6 내부 용어 — 통과. `SUBMITTED`·MSD_006 같은 값이 화면에 새지 않는다. AP-7 무력한 컨트롤 — **위반 → 고침**. 이 티켓 자체다. After DOM 에서 그 줄의 버튼 배열이 비어 있는 것을 확인했다. AP-8 수 반복 — 통과. 이 줄에 수가 없다.
- 공개 안전성: 비밀값, 실데이터, 개인정보, 내부 호스트, 로컬 경로 없음. 캡처는 합성 seed 데이터에서 촬영했다.
