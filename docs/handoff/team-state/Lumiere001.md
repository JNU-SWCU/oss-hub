...
- 공개 안전성: 비밀값, 실데이터, 개인정보, 내부 호스트, 로컬 경로 없음.
## 2026-09-05 — 다시 게시 확인창의 문구를 실제 동작에 맞춘다

- 상태: review
- Issue: #1208
- PR: 없음 (인터뷰 후 사람이 연다)
- blocker: 없음
- 내용: #1181 이 고친 `LIFECYCLE_COPY` 의 반대쪽이다. 다시 게시 확인창이 「공개 목록에 다시 노출되고」라고 약속했는데, 공개 모수가 `PUBLISHED | ARCHIVED`(`program-list-status-filter.ts`)라 내려가 있는 동안에도 목록·상세가 열려 있었으므로 새로 생기는 노출이 없다. 실제로 바뀌는 것 두 가지를 코드로 확인하고 그것만 말하게 했다 — (1) 모집 상태가 `ARCHIVED → ended` 고정에서 풀려 날짜 파생으로 돌아간다(`getProgramRecruitmentState` 와 backend `deriveProgramListStatus` 의 첫 분기가 같은 우선순위다), (2) 신규 신청이 lifecycle 거절(`applications.service.ts` 의 APP_020)에서 벗어나 바로 다음 줄의 신청 기간 검사만 받는다. `program-lifecycle.service.ts` 의 재게시는 `lifecycle` 한 칸 갱신과 감사 로그가 전부라 다른 부수 효과는 없다. 동작은 한 줄도 건드리지 않았고 내리기 쪽 문구도 그대로 두었다.
- 결정: 문구에 「종료」 배지를 다시 넣지 않았다. 커밋 `0131b9d0` 이 내리기 쪽에서 같은 약속을 걷어냈기 때문이다 — `getProgramListBadge` 가 신청이 있는 학생 카드에서 지원 상태를 모집 배지보다 앞에 두므로 「종료」로 보인다고 말할 수 없다. 대신 배지 이름을 부르지 않고 「모집 상태가 내림에 고정돼 있던 것이 풀린다」고만 적었고, 그 금지를 `not.toContain('「종료」')` 로 테스트에 함께 고정했다.
- 검증: frontend lint 0 errors(warning 5건은 이 브랜치와 무관한 기존 항목)·typecheck 통과·335 파일 3376 테스트 통과. 같은 워크트리에서 두 파일만 `origin/main` 으로 되돌려 잰 기준선이 335 파일 3375 테스트라, 늘어난 1건이 이 티켓이 넣은 테스트다(HEAD == origin/main `e1888482`, fetch 후 behind 0 확인). 대조 실험 두 갈래를 돌렸다 — (가) `program-edit-lifecycle-section.tsx` 만 `git checkout origin/main` 으로 되돌리면 새 테스트 1건이 정확히 실패하고 나머지 24건은 통과, (나) 문구를 「공개 목록에 다시 노출되며 신청이 다시 열립니다」라는 **다른 표현의 같은 거짓말**로 바꾸면 `not.toContain('다시 노출')` 가지가 실패한다. 둘 다 확인 후 원복하고 `git status` 로 확인했다. 로컬 리뷰 하네스(포트 3218)에서 확인창을 실제로 열어 1440x900·390x844 Before/After 캡처, console error 0.
- 남은 것: 로컬 리뷰 픽스처에는 교직원이 편집할 수 있는 ARCHIVED 프로그램이 없다. 앞선 초안이 `staff-program-fixtures.ts` 의 `CAPSTONE_PROGRAM` 을 ARCHIVED 로 바꿔 두었는데, 그 프로그램은 「대시보드 카드 전체 0」을 보는 자리라 공유 픽스처를 바꾸면 다른 리뷰가 보는 화면까지 달라진다. 이 티켓 계약이 문구만이라 되돌렸고, 캡처는 임시로 뒤집어 찍은 뒤 원복했다. 상시로 볼 자리가 필요하면 전용 픽스처를 새로 추가하는 별도 티켓이 맞다.
- 공개 안전성: 비밀값, 실데이터, 개인정보, 내부 호스트, 로컬 경로 없음.

## 2026-09-05 — 다시 게시 확인창 문구의 남은 거짓 두 곳을 검토에서 잡아 고친다

- 상태: review
- Issue: #1208
- PR: 없음 (인터뷰 후 사람이 연다)
- blocker: 없음
- 내용: 같은 티켓의 앞 커밋(`511bfd5b`)이 넣은 새 문구를 반증 검토하다가, 고친 문구 안에 같은 종류의 거짓이 두 개 남아 있는 것을 찾아 고쳤다. (1) 조건절이 「신청 기간이 남아 있으면」이었다 — 아직 시작하지 않은 신청 기간(upcoming)까지 포함해 읽히는데, 그 프로그램은 다시 게시해도 `applications.service.ts` 의 `now < program.applicationStartAt` 에 계속 막히므로 「곧바로 열린다」가 거짓이 된다. 원문이 쓰던 정확한 표현 「신청 기간 안이면」으로 되돌렸다. (2) 「모집 상태도 내림에 고정돼 있던 것이 풀려」는 게시 축의 값(「내림」)을 모집 축의 값인 것처럼 불렀다 — 모집 상태가 고정돼 있던 값은 종료이고 내림은 그 원인이다. 티켓이 「종료」와 「내림」의 표시 구분을 범위 밖으로 두었으므로 값을 부르지 않고 원인만 말하는 「내려 둔 동안 고정돼 있던 모집 상태도 풀려」로 바꿨다. 문장 순서도 내리기 쪽 짝(`PUBLISHED.dialogDescription`)과 같은 「바뀌는 것 → 다만 안 바뀌는 것 → 되돌릴 수 있음」으로 맞췄다.
- 결정: 문구만 고치고 동작·픽스처·내리기 쪽은 그대로 두었다. 앞 커밋이 판단한 것(「종료」 배지를 약속하지 않는다, 공유 픽스처를 바꾸지 않는다)은 코드로 다시 확인하고 유지했다 — `findStaffProgram` 이 여는 교직원 편집 픽스처 네 개가 모두 `PUBLISHED` 인 것도 확인했으므로, 상설 ARCHIVED 자리가 필요하면 여전히 별도 티켓이다.
- 검증: frontend 335 파일 3376 테스트 통과 · typecheck 통과 · lint 0 errors(warning 5건은 `sidebar-drawer.test.tsx` 의 기존 항목으로 이 브랜치가 건드리지 않는다) · `prettier --check .` 통과. 대조 실험을 새 문구로 다시 돌렸다 — (가) `program-edit-lifecycle-section.tsx` 만 `git checkout origin/main` 으로 되돌리면(그 파일이 `origin/main` 에 있음을 `git cat-file -e` 로 먼저 확인) 새 테스트 1건만 실패하고 24건은 통과, (나) 정확일치 긍정 단언을 임시로 걷어낸 뒤 문구를 네 가지 거짓으로 각각 바꿔 `not.toContain` 네 가지(`다시 노출`·`목록에 다시`·`「종료」`·`신청 기간이 남아 있`)가 **각각 다른 줄에서** 실패하는 것을 하나씩 확인했고, 진짜 문구로는 25건 전부 통과했다. 실험 뒤 매번 원복하고 `git status` 로 확인했다. 로컬 리뷰 하네스(포트 3218, 촬영 후 종료·포트 해제 확인)에서 확인창을 다시 열어 1440x900·390x844 After 캡처를 새 문구로 다시 찍었고 console error 0이다. 캡처를 위해 `CAPSTONE_PROGRAM` 을 임시로 ARCHIVED 로 뒤집었다가 원복해 네 픽스처 전부 `PUBLISHED` 인 것을 확인했다.
- 남은 것: 배포 환경 확인은 머지·배포 이후에만 의미가 있어 못 했다. UX 안티패턴 여덟 항목 판정 표는 PR 본문에 들어가야 하는데 이 작업은 PR을 열지 않으므로 사람이 열 때 넣는다.
- 공개 안전성: 비밀값, 실데이터, 개인정보, 내부 호스트, 로컬 경로 없음.
