...
## 2026-09-17 — 우리 팀 저장소 URL 조회와 팀장 수정

- 상태: review
- Issue: - (사용자 직접 요청)
- PR: (이 PR)
- 우리 팀 화면에서 기존 저장소 편집기를 재사용하고 서버가 반환하는 수정 권한을 따른다.
- 최초 신청자는 권한 근거가 아니며 현재 팀장만 수정한다. 저장 시 Team 행을 잠근 뒤 팀장·멤버십을 다시 읽어 탈퇴·승계와 직렬화한다.
- 기존 승인·종료일 제한, 변경 이력, 이전 저장소 수집 데이터 보존 계약은 유지한다.
- blocker: 없음

## 2026-09-19 — 우리 팀 저장소 카드 디자인 리뷰 반영

- 상태: review
- Issue: - (사용자 직접 요청)
- PR: [#1287](https://github.com/JNU-SWCU/oss-hub/pull/1287)
- 큰 제목 배경과 별도 수정 버튼 줄을 제거하고 팀 구성원 카드와 같은 중립 색상·16px 여백으로 줄였다.
- 연필·체크·X 버튼은 접근성 이름과 44px 조작 영역을 유지한다.
- 수정 권한, 변경 사유, 취소 확인, 저장 실패·결과 불명 처리의 계약은 바꾸지 않는다.
- 기존 텍스트 기반 테스트 선택자를 접근성 이름 기준으로 바꾸고 모바일 편집 취소 후 재진입을 E2E에서 확인한다.
- blocker: 없음
