...
## 2026-09-15 — QA170 목록 카드 통합

- 상태: active
- Issue: #1245
- PR: (이 PR)
- 공용 `ListCard`가 Card primitive 위에서 카드 전체 링크와 배지·제목·부제·메타·행동 자리를 소유한다. `ProgramCard`는 기존 상태 매핑·이미지 입력·접근성 설명을 연결하고 아카이브 목록도 같은 조합을 쓴다.
- `data-slot="program-card"`를 유지해 목록·종료 카드·그리드 회귀 selector를 보존했다.
- QA91(#1080)의 배지·기간 문구는 바꾸지 않았다. #1281과 겹치는 ProgramCover 입력은 그 PR의 `programCoverSource` 변경을 병합 시 보존해야 한다.
- blocker: 배포 후 두 목록의 키보드·390px 재확인은 병합·배포 뒤 이슈에서 확인한다.
