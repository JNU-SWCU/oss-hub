...
| 환경 변수 응집·배포 파이프라인 단순화 | @GoBeromsu | active | [#305](https://github.com/JNU-SWCU/oss-hub/issues/305) | [#306](https://github.com/JNU-SWCU/oss-hub/pull/306)·[#307](https://github.com/JNU-SWCU/oss-hub/pull/307)·[#309](https://github.com/JNU-SWCU/oss-hub/pull/309)·[#310](https://github.com/JNU-SWCU/oss-hub/pull/310) merged · [#311](https://github.com/JNU-SWCU/oss-hub/pull/311) active | PR-1 ADR-002 개정 병합 완료(`d4a688f`, merge-policy `PASS` HIGH_RISK) · PR-2a env 공용 계약 병합 완료(`f27bdeb`, 독립 리뷰 4회 blocker 0): 운영 App 자격증명 compose 매핑(F6) · 죽은 키 제거와 빌드 주입값 분리(F7) · checker를 TypeScript AST·`docker compose config` 정규화 기반으로 재작성하고 승인 helper 면제를 유일한 top-level 선언으로 한정(`#309`, `#310`)해 61 fixture pass(x- 확장 spoof·승인 경로 밖 동명 helper·중첩 함수 스코프 면제 전파·여러 줄 구조 분해·`.mts`/`.cts`·parse 오류·스캔 대상 부재를 fail-closed 검출, 주석·백틱 오탐 제거) · PR-2b typed RuntimeConfig 선행으로 canonical 획득 경로의 CLI 전용 키 면제를 기존 소비 경로와 동일하게 한정 · Operations App 자격증명은 `:?` 유지(선택값 전환은 `ADR-006` 축소 동작이 승인 경로에 미구현이라 revert, capability 배선은 후속 고위험 작업) · `ci.yml` 인라인 필수 키 사본 제거 후 required job 배선 | ralplan consensus: Architect `CLEAR`/`APPROVE`, Critic 조건부 `ITERATE` · 공용 경로 선점 선언 완료 · public-safe 파일명 가드에 자격증명 메모(`credentials.md`·`secrets.yaml`·`creds.txt` 등) 차단 추가 · 체크리스트 C1 repo측 negative fixture와 credential preflight 종결(운영 환경의 Collection App·S3 필수 키 등록 완료, Operations App 자격증명은 App 생성 전이라 미등록) · C4 backup retention 실측으로 N=120 승인 기록 완료(`#305`) · 운영 컨테이너 0개·`minio_data` 볼륨 부재로 서비스 미기동 상태 · pre-merge accept·legacy Jenkins 복원은 점검 창으로 이월 · tag ruleset은 범위 제외(태그 조작 방어는 `RELEASE_ACCEPT role=PM` 승인 바인딩이 담당) · PM 단독 accept 전환(#274 계열)과 운영 MinIO 추가(#302·#303) 반영 완료 |
