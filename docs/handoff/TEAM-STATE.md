...
| 셸 nav 아카이브 추가 + 히어로 내부 반전 nav(`[data-surface='inverted']` 토큰 스코프) | @GoBeromsu | review | [#276](https://github.com/JNU-SWCU/oss-hub/issues/276) | - | frontend 83 files/503 tests pass(신규 `shell-nav.test.tsx` 3건 포함) · lint/typecheck/build(`/archive` 정적 생성 확인 · 신규 `ShellNav`가 라우트 21개 전부 정상 빌드)/format:check pass | `LoginButton`·`SessionEntryNavLink`의 `Button variant="ghost"` 기본(비-hover) 텍스트 색이 반전 스코프로 재색칠되지 않음 — ghost 변형이 `hover:text-foreground`만 선언해 hover 시에는 반전되지만 기본 상태는 조상에서 이미 계산된 색을 상속함(정적 CSS 케스케이드 분석으로 확인). 브라우저 확장 미연결로 실제 렌더 스크린샷은 미확인(unblock: @GoBeromsu 또는 사람 QA) |
