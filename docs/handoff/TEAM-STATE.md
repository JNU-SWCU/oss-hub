...
| 에이전트 병합 게이트 — check run 발행 idempotent화(required 등재 선행 조건) | @GoBeromsu | review | [#384](https://github.com/JNU-SWCU/oss-hub/issues/384) | - | 판정기 fixture 56/56 local pass(발행 계획 9건 신규) · `--simulate` dry-run 정상 · 기존 판정 47건 무변경 통과 | 없음 — branch protection required 등재는 실 PR에서 갱신 결정론성 관찰 후 별건(unblock: @GoBeromsu) |
