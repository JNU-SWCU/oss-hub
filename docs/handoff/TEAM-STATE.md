...
| 환경 변수 응집·배포 파이프라인 단순화 | @GoBeromsu | active | [#305](https://github.com/JNU-SWCU/oss-hub/issues/305) | - | PR-1 ADR-002 개정 완료(최신 main 위 재작성) · prettier·public-safe·team-state 게이트 pass · architect BLOCK 2건(운영 MinIO 토폴로지·보존 정책) 반영 | ralplan consensus: Architect `CLEAR`/`APPROVE`, Critic 조건부 `ITERATE`(착수 전 체크리스트 closure가 approval gate) · 공용 경로 선점 선언 완료 · 체크리스트 4건(credential preflight·pre-merge accept·legacy Jenkins 복원·backup N) 미종결 · tag ruleset은 범위 제외 · PM 단독 accept 전환(#274 계열)과 운영 MinIO 추가(#302·#303)를 ADR에 반영 |
