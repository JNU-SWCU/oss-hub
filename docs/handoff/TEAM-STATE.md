...
| 에이전트 병합 게이트 — jenkins script 추출 경로에 배포 승인 보호 확장 | @GoBeromsu | review | [#387](https://github.com/JNU-SWCU/oss-hub/issues/387) | - | 판정기 fixture 52/52 local pass(신규 5건) · `scripts/jenkins/**` 재귀 PM 전속·인접 경로 비매칭 확인 | 없음 — 이 보호 확장이 병합된 뒤에만 P1 추출 PR을 시작한다 |
