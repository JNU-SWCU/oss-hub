...
| GitHub App 개인키 파일 입력 전환 · 호스트 hot reload dev 계약 | @GoBeromsu | active | [#366](https://github.com/JNU-SWCU/oss-hub/issues/366) | Wave 1 [#367](https://github.com/JNU-SWCU/oss-hub/pull/367) merged · Wave 2 PR pending | Wave 1 public-safe 회귀 37/37 · Wave 2 coverage 63/63(신규 스캔 경계 2건)·backend health 200·HMR 재시작 관측 · Wave 4 드릴 8항목 실측 완료 | 7 wave 중 1 merged·2 제출·4(드릴) 완료 · 드릴 결론은 `SECRETS_DIR` `jenkins:1000` `2750`(setgid)+파일 `0640` — jenkins(uid 105)에 chown 권한이 없어 계획의 `1000:1000 0600`은 실현 불가, symlink 교체는 실행 중 컨테이너에 반영되지 않아 회전 시 force-recreate 필수 · Wave 3(backend `collection`·`repositories` 파일 로더)은 @Lumiere001 소유 경로 변경이라 free-role 예외로 진행(AGENTS.md §3, 원본 ADR-005) · Wave 3·5는 배포 계약 경로라 exact head/base `PM_ACCEPT` 필요 · Wave 5 착수 전 Jenkins file credential 2건 등록 + `SECRETS_DIR` root 1회 준비 필요(unblock: @GoBeromsu) · 종점은 R1(호환)·R2(활성화) 릴리즈 2회 실배포 |
