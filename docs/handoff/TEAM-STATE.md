...
| 환경 변수 응집·배포 파이프라인 단순화 | @GoBeromsu | active | [#305](https://github.com/JNU-SWCU/oss-hub/issues/305) | [#306](https://github.com/JNU-SWCU/oss-hub/pull/306) merged | PR-1 ADR-002 개정 병합 완료(`d4a688f`, merge-policy `PASS` HIGH_RISK) · PR-2a env 공용 계약 진행 중: 운영 App 자격증명 compose 매핑(F6) · 죽은 키 제거와 빌드 주입값 분리(F7) · checker를 선언·소유 서비스 매핑·스캔 문법 삼중 계약으로 분리해 19 fixture pass(F6 키별 독립 negative fixture 포함) · `ci.yml` 인라인 필수 키 사본 제거 후 required job 배선 | ralplan consensus: Architect `CLEAR`/`APPROVE`, Critic 조건부 `ITERATE` · 공용 경로 선점 선언 완료 · 체크리스트 4건 중 C1 repo측 negative fixture는 종결, credential preflight·pre-merge accept·legacy Jenkins 복원·backup N 실측은 서버 접근이 필요해 점검 창으로 이월 · tag ruleset은 범위 제외(태그 조작 방어는 `RELEASE_ACCEPT role=PM` 승인 바인딩이 담당) · PM 단독 accept 전환(#274 계열)과 운영 MinIO 추가(#302·#303) 반영 완료 |
