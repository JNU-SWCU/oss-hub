...
| 환경 변수 응집·배포 파이프라인 단순화 | @GoBeromsu | active | [#305](https://github.com/JNU-SWCU/oss-hub/issues/305) | - | 미착수(최신 main 위 재작성 예정) | ralplan consensus 승인(Architect `CLEAR`/`APPROVE`) · 공용 경로 선점 선언 완료 · 착수 전 체크리스트 4건(credential preflight·pre-merge accept·legacy Jenkins 복원·backup N) 미종결 · tag ruleset은 범위 제외 · PM 단독 accept 전환(#274)과 운영 MinIO 추가(#302·#303) 반영해 범위 재산정 필요 |
