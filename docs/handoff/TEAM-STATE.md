...
| 검사기 단순화 — 환경변수 소비 지점을 manifest 하나로 lint 강제 | @GoBeromsu | review | [#381](https://github.com/JNU-SWCU/oss-hub/issues/381) | - | backend lint exit 0(기존 위반 0건)·typecheck exit 0 · RED 4형태(멤버·계산·구조분해·동적) 발화 · GREEN 4건(인자 전달 2·spec 예외·runtime-config 예외) 통과 | 없음 — 후속 env 계약 검사기 AST 스캔 제거가 이 규칙에 의존한다 |
