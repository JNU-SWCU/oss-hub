...
- 두 번째 패널의 수치는 공개 그래프에서 센 값이다. 0이면 숫자 대신 `—`를 쓰고, 출처 배지로 `공개 아카이브 기준` · `예시 데이터 기준` · `공개 집계 준비 중`을 구분한다. 없는 값을 0으로 채우지 않는다.
...
- canvas 배율은 `min(devicePixelRatio × 품질계수, 2)`다. 품질계수는 1 → 0.75 → 0.5 세 단계이고, 12프레임 표본의 p95가 예산을 넘으면 한 단계 내린다. 한 번 내린 뒤 자동으로 올리지는 않는다.
...
| 조회 실패 | 버튼을 만들지 않는다 | 여정 아래 본문에서 오류와 재시도를 보인다 |
