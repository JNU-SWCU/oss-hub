...
| 왼쪽 사이드 패널 | **컨텍스트형** — 프로그램 `?status=` · 아카이브 `?category=` · 랭킹 `?year=` · 대시보드 역할 메뉴(학생: 저장소·활동 / 교직원: 운영 대시보드 / 관리자: 감사·시스템). 필터는 flat 피어(네스트 트리 금지). 카운트 뱃지 0도 표시. 프로그램 생성은 교직원·관리자의 프로그램 목록 `PageHeader` CTA 한 곳에서 제공한다 |
