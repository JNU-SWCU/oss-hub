...
**공통 상단 `NavBar`(`ShellNav`)를 랜딩과 업무가 재사용**한다. 색 톤만 갈린다.  
가입 완료 회원 업무 화면에는 그 아래 **왼쪽 “내 상황” 사이드바**를 단다.

| 항목 | 결정 |
| --- | --- |
| 바탕 | 순백색을 유지한다 |
| 상단 Nav | 전 화면 공통. 프로그램·공개 아카이브·랭킹(`public-menus.ts`) — **입구** |
| 왼쪽 사이드 패널 | **프로그램 메뉴** 탑다운(전체·모집중·진행중·접수대기·종료, `?status=`). 가입 완료 시 **내 상황** 역할 홈 추가. 아카이브·랭킹은 상단만 |
| 레이아웃 | 전체 폭 Nav 아래 `[사이드 패널 \| 본문]` (L1). 랜딩·가입은 패널 없음 |
