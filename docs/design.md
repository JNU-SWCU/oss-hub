...
| 위험/오류 | `--destructive` | 라이트 `--palette-red-600` (`#c02626`) · 다크 `--palette-red-400` (`#e26a6a`) | 상태 표시 보조 램프. 표면별로 다른 단계를 참조한다 — 아래 명도비 항목 참고 |
...
| 표면 | 참조 | 배경 | 명도비 | 판정 |
| --- | --- | --- | --- | --- |
| 라이트(`:root`, 반전 안 표면) | `--palette-red-600` | `#ffffff` | 5.92:1 | AA 통과 |
| 다크(`.dark`) | `--palette-red-400` | `#1a1a1a` | 5.39:1 | AA 통과 |
| (이전) 양쪽 공통 | `--palette-red-500` | 각각 | 4.38:1 / 3.98:1 | **양쪽 모두 미달** |

`--destructive`는 텍스트 색과 버튼 배경(흰 전경) 양쪽으로 쓰이는데, 두 경우의 명도비 계산 결과는 같으므로 위 값 하나로 판정한다.

새 semantic 토큰을 추가할 때는 라이트·다크 **양쪽 배경 기준으로 각각** 4.5:1을 확인한다. 한쪽만 확인하고 같은 palette 단계를 양쪽에 쓰면 이 결함이 재발한다.
