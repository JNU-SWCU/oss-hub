...
개수는 화면에 **보이는 오류 줄 수**다 — 시작·종료를 한 줄로 보이는 기간은 하나로 세고, 저장 버튼 옆에 따로 서는 일반 오류와 창 안에 뜨는 오류는 세지 않으며, 칸이 스스로 띄운 오류(대표 이미지의 파일 오류, 마일스톤 제출 항목 칸의 이름·파일 오류)는 센다. 한 칸이 같은 문장을 두 자리에 띄우면 하나로 센다.
요약은 제출(저장·계속·가입 마치기)을 누른 뒤부터 선다 — 치는 도중이나 파일을 고를 때 칸 옆에 먼저 뜨는 오류가 둘 이상이어도 누르기 전에는 요약을 그리지 않는다. 그러지 않으면 입력하는 동안 맨 위 상자가 나타났다 사라지기를 되풀이하고, `role="alert"`라 낭독기가 그때마다 읽던 것을 끊는다.
...
Slots·Props: FormErrorSummaryProps의 `count`·`surface`·`className`을 쓴다. 개수 계산은 각 폼이 하고, 내부 오류 목록의 길이가 아니라 보이는 오류 줄 수(R-16)를 넘긴다. 가입 무대처럼 `data-surface="inverted"` 안에서는 `surface="inverted"`를 넘긴다 — 그 스코프는 `--destructive`를 밝은 붉은색으로 덮지만 `--card`는 덮지 않아, 기본 모양이면 흰 상자 위에 옅은 글자가 선다.
