...
- canvas 배율은 `min(devicePixelRatio × 품질계수, 2)`다. 품질계수는 1 → 0.75 → 0.5 세 단계이고, 12프레임 표본의 p95가 예산을 넘으면 한 창 만에 한 단계 내린다. 올릴 때는 기준을 비대칭으로 둬서, p95가 예산의 70% 아래로 들어온 창이 세 번(36프레임) 연속돼야 한 단계 올린다. 그 사이 예산을 넘거나 70%와 예산 사이에 걸친 창이 하나라도 끼면 연속 기록은 0으로 돌아간다. 여유선과 연속 조건이 두 단계 사이를 오가는 출렁임을 막는 장치다. 오르내림은 항상 한 단계씩이다.
...
| 익명 | `GitHub으로 로그인` — 로그인 실패 뒤에는 `GitHub 로그인 다시 시도` | `githubLoginPath` |
| 로그인·역할 미확정 | `가입 계속하기` | `/consent` |
| 학생 | `내 대시보드` | `/dashboard` |
...
| 한 화면에 담는다 | 위쪽 공통 셸과 온보딩 단계 표시가 쓰는 높이를 뺀 `min-h-[calc(100dvh-9rem)]`을 쓴다 |

라디오는 `sr-only` peer이고 라벨 전체가 클릭 영역이다. 포커스 링은 `focus-within`으로 카드에 걸리고, 제출 버튼은 `min-h-11 w-full`로 터치 타깃 44px을 확보한다.

### 검토 중인 방향 — 업무 화면 미감 개편

> ⚠ 아래는 아직 **확정되지 않은 방향**이다. 대시보드·목록·설정 등 업무 화면의 미감 개편은 현재 HTML 시안 단계이고 사용자 판단을 기다리는 중이다. 이 절의 내용으로 구현을 시작하지 않는다.
...
간격·타이포 스케일·카드 스타일·표 밀도 등 나머지는 미정이다. 확정되면 이 절을 정식 계약으로 옮겨 쓴다.
