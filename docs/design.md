...
폼 맨 위에 오류 요약(목록이든 개수든)을 두지 않는다 — 이 저장소의 폼은 칸이 많지 않아 한눈에 들어오고, 틀린 칸마다 빨간 테두리와 문구가 이미 서 있어서 요약은 같은 정보를 한 번 더 말한다.
...
2026-09-25 결정(PM, #1416 검토): 상단 요약을 두지 않는다. 그 전의 두 문구 — 긴 폼에만 요약(#1401), 오류가 둘 이상이면 개수 요약(#1406) — 는 이 결정으로 대체됐다. #1406이 잰 「아래쪽 칸만 틀리면 첫 오류가 화면 밖에 있다」는 문제는 제출 직후 첫 오류 칸에 거는 `scrollIntoView({ block: 'center' })`가 이미 풀고 있다(그때 측정 10회 모두 자동 스크롤 뒤에는 오류가 화면 안에 보였다).
