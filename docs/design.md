...
**R-41** 상태 라벨은 **상태 명사**로 쓴다.
한 컨트롤의 선택지끼리, 그리고 나란히 선 컨트롤끼리 형태를 섞지 않는다 — `있음 · 없음`, `활성 · 비활성`처럼 같은 종류의 말로 맞춘다.
`-됨`·`-함` 같은 서술형은 상태가 아니라 사건을 말하므로 고를 수 있는 값의 이름으로 쓰지 않고, 행동(동사)은 버튼이 맡는다.
지나간 사건을 그 자체로 보여 주는 이력 목록은 이 규칙의 대상이 아니다.
...
| 접근 | GRANTED · NONE | 있음 · 없음 | 없음 | 교직원·관리자 접근을 지금 가졌는지. 「없음」은 받은 적 없는 계정과 회수된 계정 모두에 맞는 중립적인 말이다. 배지를 붙이지 않는다 — 머리말의 계정 상태 배지가 같은 사실을 이미 말한다(#1365). 가진 쪽은 「허용됨」이 아니라 「있음」이다 — 같은 카드의 계정 상태가 `활성 · 비활성`이라 한쪽만 서술형이면 품사가 섞인다(R-41) |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| field | 필드 단위 검증 실패 | error | FieldError | 컨트롤 바로 아래 | `role="alert"` | 첫 오류로 포커스 이동 | 사용자가 고칠 때까지 지속 | 없음 — 첫 오류 칸으로 옮겨 간 포커스가 다음 행동이다. 폼 맨 위 요약은 두지 않는다(R-16) |
...
폼 맨 위에 오류 요약(목록이든 개수든)을 두지 않는다 — 대부분의 폼은 칸이 많지 않아 한눈에 들어오고, 틀린 칸마다 빨간 테두리와 문구가 이미 서 있어서 요약은 같은 정보를 한 번 더 말한다(PM 판단).
한 화면을 넘는 긴 폼(프로그램 만들기·편집)에서도 요약 대신 첫 오류 칸으로 포커스를 옮겨 화면 가운데로 스크롤하고, 나머지 오류는 각자 칸에서 빨간 테두리와 문구로 남는다.
...
2026-09-25 결정(PM, #1416 검토): 상단 요약을 두지 않는다. 그 전의 두 문구 — 긴 폼에만 요약(#1401), 오류가 둘 이상이면 개수 요약(#1406) — 는 이 결정으로 대체됐다. #1406이 잰 「아래쪽 칸만 틀리면 첫 오류가 화면 밖에 있다」는 문제는 제출 직후 첫 오류 칸에 거는 `scrollIntoView({ block: 'center' })`가 이미 풀고 있다(그때 측정 10회 모두 자동 스크롤 뒤에는 첫 오류가 화면 안에 보였다). 그 아래 남은 오류는 요약이 없으므로 화면을 내려 가며 각 칸에서 확인한다 — 이것이 이 결정이 받아들인 비용이다.
...
| AP-19 | 품사가 섞인 선택지 | `lib/status-vocabulary/**`와 `<option>`·`FilterChip` 라벨에 `-됨`·`-함` 서술형과 상태 명사가 한 묶음 안에 섞임 | R-41 |
...
| 2026-09-24 | 오류가 둘 이상이어도 개수를 적은 상단 요약이 어느 폼에도 없다 — 프로그램 만들기(`programs/new` 기본 정보 단계)·프로그램 편집이 필수값을 모두 비우면 각각 오류 4개, 게시판 글 작성·편집이 각각 2개(제목·본문)를 필드 옆에만 보인다 | R-16 | 2026-09-25 해소 — 규칙이 바뀌어 위반이 아니다. 상단 요약을 두지 않기로 했다(R-16, PM 결정). 같은 조사에서 첫 오류 칸 포커스가 없던 세 폼(설정·관리자의 프로필 수정 카드·프로그램 만들기의 마일스톤 추가 창)에는 이 결정과 함께 포커스를 넣었다 |
...
- 프로그램 생성·편집의 기본 정보는 프로그램명 다음에 대표 이미지 입력을 둔다.
  `기존 공지로 빠르게 시작하기`는 기본 정보 제목 옆의 보조 행동이며, 좁은 화면에서는 제목 아래로 내려온다.
  공지 주소 입력·불러오기·항목 선택은 기존 다이얼로그에서 처리하고, 선택한 내용을 적용하기 전에는 작성 중인 값을 바꾸지 않는다.
  대표 이미지 필드는 라벨 아래 전체 폭의 점선 영역에 아이콘·업로드 제목·허용 형식과 용량·선택 버튼을 위에서 아래로 배치한다.
  선택 후에는 같은 영역에서 이미지 전체를 보여 주고 파일명 또는 공지 출처, 교체·제거 행동을 이어서 제공하며, 페이지 제목 옆에 같은 미리보기를 반복하지 않는다.
  기존 Field·Button·ProgramCover와 semantic 토큰을 사용하고, JPG·PNG 및 5 MB 제한과 저장 시점은 유지한다.
...
- 학생은 한 제출 화면에서 내용만, 파일만, 내용과 파일을 함께 낼 수 있다.
  두 값이 모두 비었거나 선택한 파일 오류가 해결되지 않았으면 제출을 막고, `내용이나 파일을 하나 이상 추가해 주세요.`와 `둘 다 추가해도 됩니다.`를 서로 다른 문장으로 안내한다.
- 파일을 받는 자리는 고르기 **전에** 허용 형식과 상한을 한 줄로 말하고 같은 목록을 `accept`에 건다.
  학생 제출은 `PDF, HWP, ZIP · 최대 5 MB`이고, 교직원 양식 올리기(프로그램 상세·마일스톤 편집)는 이미지 양식을 함께 받으므로 `PDF, HWP, JPG, PNG, ZIP · 최대 5 MB`이다.
  두 자리는 문장 형식은 같고 형식 목록만 각자의 정책에서 만든다 — 한 문장을 공유하면 한쪽 정책이 바뀔 때 다른 쪽이 거짓말을 한다.
  숫자와 형식 목록은 서류 목록 또는 마일스톤 편집 응답의 `fileUpload`를 그대로 읽는다.
  상한을 넘거나 허용 형식 밖인 파일은 실제 File 대신 이름과 거절 사유만 남긴다.
  파일 입력은 계속 사용할 수 있으며 다른 파일을 바로 선택하거나 `파일 없이 계속`으로 오류만 해제한다.
  두 복구 동작 모두 작성한 글을 보존하고 자동 제출하지 않는다.
  기존 첨부가 있다는 사실은 오류 중에도 표시하며, 새 파일이 없을 때 최신 제출본에서 빠진다는 안내를 표시한다.
  마감 후 최종 재제출의 기존 확인창 하나에도 빠질 파일명을 표시하며 마감 전 수정에 추가 확인창을 만들지 않는다.
  단위 표기는 `MB` 하나만 쓴다(실제 상한은 5 MiB다). 서버가 내는 거절 문구도 같은 문장이다.
- 프로그램 서류 현황과 마일스톤 수합 화면의 ZIP 범위 선택은 내려받기 아이콘으로 펼치고 닫기 아이콘으로 접는다.
  두 상태에 맞는 접근 가능한 이름과 툴팁을 제공하고 실제 다운로드 버튼은 가시 텍스트를 유지한다.
  프로그램 전체·마일스톤·팀과 팀별·서류별 폴더를 선택하고, 현재 제출 글과 파일만 포함하며 이전 이력은 제외한다는 요약 뒤 바로 다운로드한다.
  마일스톤에서 열면 그 마일스톤을 처음 선택하고 기존 수합 표의 항목별 ZIP과 묶기 조작은 유지한다.
  목록 검색·필터와 다운로드 범위는 별개이며 미리보기 API 없이 파일 수나 제외 수를 추정하지 않는다.
  다운로드 실패는 선택을 보존하고 같은 동작으로 재시도하며 별도의 재확인 팝업은 만들지 않는다.
...
학생이 팀을 만들고 팀원을 초대하고 프로그램에 신청하는 화면군의 상호작용 계약이다.
용어는 표준 용어로만 쓴다 — 맥락 액션(contextual action), 점진적 공개(progressive disclosure), 시각적 위계(visual hierarchy), 의미적 어포던스(semantic affordance), 단일 페이지 폼(single-page form).
이 절은 프로그램 만들기(교직원)의 단계 이동 결정을 바꾸지 않으며 학생 신청 화면에만 적용된다.

**R-26 — 맥락 액션**: 팀원 초대 입구는 팀원 목록(roster) 안에 둔다.
초대 입력은 공유 검색 모달 하나이며 목록 화면과 신청 화면이 같은 모달을 재사용한다.
`PageHeader`에 초대 토글이나 초대 폼 열기/닫기 스위치를 두지 않는다 — 대상이 되는 목록에서 먼 자리에 액션을 놓으면 무엇에 대한 초대인지가 사라진다.
규정 대상은 트리거의 자리이며 모달의 구현 위치는 아니다 — portal로 띄우거나 열림 상태를 부모 컨트롤러·화면 상태가 소유해도 roster 안의 트리거로 열리면 이 규칙을 지키는 것이다.
검출 신호: roster 밖에 놓인 초대 트리거 — `PageHeader`·페이지 상단 액션 영역의 초대 버튼과 초대 토글 state.

**R-27 — 점진적 공개**: 행의 보조 액션(역할 변경, 초대 취소, 내보내기 등)은 아이콘 버튼 또는 행 메뉴로 접고 툴팁을 붙인다.
접근 가능한 이름은 행마다 고유해야 한다(`초대 취소` 아님, `{이름} 초대 취소`).
되돌릴 수 없는 액션의 확인 다이얼로그는 접었다는 이유로 없애지 않는다(피드백·알림 표의 dialog 행).
검출 신호: 같은 접근 가능한 이름이 여러 행에 반복, 아이콘 전용 버튼에 `aria-label`·툴팁 부재, 확인 없이 즉시 실행되는 파괴적 행 액션.

**R-28 — 단일 페이지 폼과 최종 확정**: 신청은 한 화면에서 프로필 확인·팀 구성·저장소 연결·동의를 함께 다루고 마지막에 신청 확인 하나로 끝난다.
작성 중에 임의의 단계 위저드, 화면 새로고침 요구, 팀 삭제 같은 파괴적 컨트롤을 섞지 않는다.
제출·확정·취소 같은 주 액션 버튼은 가시 텍스트를 유지한다 — 텍스트 버튼 일반을 금지하는 규칙이 아니다.
검출 신호: 신청 화면의 단계 인덱스 state, 신청 폼 안의 팀 삭제 버튼, `window.location.reload` 호출.

**R-29 — 초대 대기 표시**: 보낸 초대는 별도 목록을 만들지 않고 같은 roster에 `초대 대기` 상태로 함께 보여 준다.
대기 초대는 그 roster에 대기 행 하나로만 들어가고 같은 사람이 수락한 팀원 행으로 또 그려지지 않으며, 수락한 팀원과 시각적으로 구분하되 인원 수·정원 계산에는 넣지 않는다.
쓰기 시점은 다음과 같다 — 화면 진입·렌더·검색어 입력은 팀 생성 쓰기를 유발하지 않는다.
초안 팀은 사용자의 명시적인 초대 진입(roster의 초대 트리거 — 현재 팀 스코프 검색이 초안을 전제로 하므로)이거나 최종 신청 확인일 때만 만들어진다.
초대 보내기는 신청 확인을 기다리지 않고 그 시점에 실제로 발송된다 — 거짓말이 되는 약속은 `모든 쓰기는 최종 확인까지 미뤄진다` 하나뿐이다.
검출 신호: `초대 대기` 항목을 담은 두 번째 목록 컴포넌트, 수락 인원 카운트에 pending 초대 포함, 렌더·검색어 변경 effect에서 발생하는 팀 생성 요청.

**R-30 — 컴팩트 목록**: 팀원·초대·제출 항목은 항목마다 카드를 반복하지 않고 `DataTable` 또는 `ListPanel`+`ListRow`의 조밀한 행으로 그린다.
검출 신호: roster·초대 목록을 감싼 `CardGrid`, 항목 하나당 `Card` 반복.

**R-31 — 상태·액션·내비게이션 분리**: 상태는 텍스트 또는 `StatusBadge`로 쓰고 버튼처럼 보이게 하지 않는다.
서버 쓰기뿐 아니라 다이얼로그 열기·행 펼치기·필터 적용처럼 현재 화면에서 무언가를 수행하는 UI 액션도 `button`이다.
다른 화면으로 가는 것만은 버튼처럼 꾸민 로컬 핸들러가 아니라 진짜 링크로 만든다.
검출 신호: 상태 문자열을 담은 `disabled` 버튼, `onClick`으로 `router.push`만 하는 버튼.

**R-32 — 의미 있는 숫자와 군더더기 문장**: 화면의 숫자는 무엇을 센 값인지 라벨로 밝힐 수 있을 때만 두고 아니면 없앤다.
제목이나 목록 개수를 다시 풀어 쓰는 안내 문장은 두지 않는다.
다만 안전장치 안내, 오류 문구의 다음 행동(R-15), 동의 문구는 군더더기가 아니므로 침묵으로 대체하지 않는다.
검출 신호: 라벨 없는 큰 숫자, 바로 위 heading·카운트를 되풀이하는 도입 문단.

**R-33 — 검증 근거**: 위 패턴을 바꾼 화면은 소스 기준 사전 리뷰 근거(`file:line - finding`)를 남긴 뒤에야 준비됨으로 보고한다.
리뷰 이후에는 실제로 렌더한 화면에서 키보드 도달성과 모바일 폭 동작을 확인한다.
PR 제출 전에는 변경된 사용자 흐름의 UX 리뷰를 반드시 완료하고, 발견한 문제를 수정한 뒤 같은 흐름을 재검증한다.
테스트 통과나 소스 리뷰만으로 실제 화면의 UX 리뷰를 대신하지 않는다.
리뷰 증거와 공개 첨부 절차는 `skills/submit-pr-evidence/SKILL.md`를 따른다.
...
| 알림 슬롯 | 첫 패널 안. 로그인 실패는 `--hero-danger` 색 Alert. 이전 로그아웃 안내 URL의 `role="status"` 상자는 호환용으로만 유지한다. 정상 로그아웃은 별도 완료 화면 없이 비로그인 홈으로 돌아온다 |
...
여정 자체는 세션과 무관하게 같고, 첫 패널과 마지막 패널의 주 버튼만 바뀐다(`app/_shell/landing-entry-action.tsx`). 홈 방문만으로 역할 홈으로 보내지 않는다. 보호된 화면에서 로그인을 시작한 경우에는 가입 완료 후 검증된 원래 경로로 한 번 돌아간다.

| 세션 상태 | 주 버튼 | 목적지 |
| --- | --- | --- |
| 확인 중 | `세션 확인 중` (비활성, `aria-busy="true"`) | — |
| 익명 | `로그인` — 로그인 실패 뒤에는 `로그인 다시 시도` | `/signup` |
| 로그인·역할 미확정 | `로그인` — 비로그인과 **같은** 버튼 | `/signup` |
| 학생 | `내 대시보드` | `/dashboard` (본문: 학생 대시보드) |
| 교직원 | `운영 대시보드` | `/dashboard` (본문: 운영 대시보드) |
| 관리자 권한만 있음 | `사용자 목록` | `/dashboard/users` |
| 조회 실패 | `로그인` — 비로그인과 **같은** 버튼을 그대로 낸다 | `/signup` |
...
순서는 (세션 조회 실패 시 오류·재시도) → `#current-programs`(현재 모집 중인 프로그램) → 참여 절차 → 하단 CTA → 푸터다. 소개를 먼저 보고 스크롤해 프로그램에 도착하는 구성을 유지한다.
...
| 익명 | `로그인이 필요합니다` 안내와 로그인·홈으로 버튼을 보인다 |
| 역할 미확정 | 요청 상태에 맞는 온보딩 경로로 이동 |
| 역할은 있으나 권한 밖 | 이동하지 않고 접근 권한 안내를 띄운다 |
| 조회 실패 | 어디로도 보내지 않고 오류·재시도를 보인다 |

권한 안내는 `aria-labelledby`로 제목을 연결한 section에 `접근 권한이 없습니다`, 한 문장의 설명, `내 화면으로 돌아가기` 버튼 하나(외곽선, `min-h-11`)를 둔다. 돌아갈 곳은 호출부가 지정한다. 전체 화면을 대신하는 지속적인 안내이므로 긴급 경고로 읽히는 `role="alert"`를 쓰지 않는다.

### 로그인·계정 변경·로그아웃

`/signup`의 주 행동은 `GitHub으로 계속하기`다. 다른 계정을 고르는 보조 행동과 GitHub 가입 링크를 같은 진입 화면에 두고, 수동 GitHub 로그아웃 안내는 계정 변경 도움말 안에 둔다. 서비스 로그아웃 성공 후에는 안내 선택지를 다시 고르게 하지 않고 비로그인 홈(`/`)으로 이동한다. 실패하면 현재 화면과 세션을 유지하며 오류를 알린다. 이전 `/logout` 주소도 홈으로 돌려보낸다.

로그인 후 복귀는 사용자가 선택한 내부 경로에만 적용한다. 쿼리·해시·작성 중인 입력값은 복구하지 않으며, 외부 주소와 인증·가입 처리 경로는 저장하지 않는다. 복귀 정보는 10분 동안 한 번만 사용하고 로그아웃 때 지운다.
...
| 표면 톤 | 상단 Nav는 전 화면 기본 흰 바. `data-surface="inverted"`는 가입 화면 본문 — 우주 바탕 경로의 `#main-content`(AppFrame, 무대 밖 인증·온보딩 게이트 문구까지)와 가입 무대(`SignupStage`) — 과 동의 창 두 곳 — 동의 필요 창의 안쪽 패널(흰 화면인 `/onboarding/pending`에서도 뜬다)과 약관 전문 창 전체 — 에만 둔다. 이 스코프가 덮는 var는 background·foreground·muted(-foreground)·accent(-foreground)·border·card(-foreground)·ring·destructive(-on-tint)다. `--popover`·`--primary`는 덮지 않는다 — 학과 select의 열린 목록과 흰 주 버튼의 남색 글자가 `:root` 값에 기댄다 |
