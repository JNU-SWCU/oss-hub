...
- 신청·운영·마일스톤은 왼쪽 순서에서 정할 일정을 고르고, 오른쪽의 같은 달력에서 시작일과 종료일을 차례로 눌러 정한다.
  별도의 날짜 입력 화면으로 이동시키지 않는다.
  신청은 파랑, 운영은 초록, 마일스톤은 주황으로 표시하며 연속 기간은 날짜마다 끊긴 칩이 아니라 주 경계에서 이어지는 막대로 그린다.
- 마일스톤 달력은 운영 기간 밖 날짜를 비활성화한다.
  첫 날짜를 고르면 시작은 `00:00`, 마감은 `23:59`로 명시해 만들고, 날짜를 다시 골라도 사용자가 바꾼 기존 시각은 유지한다.
  시작·마감 시각 입력은 선택한 일정 아래에 항상 보이며 `세부 일정 설정` 같은 열기·닫기 단계를 두지 않는다.
  고른 날짜와 시각은 `YYYY년 M월 D일 (요일) HH:mm` 형식의 한국어 문장으로 다시 적는다.
- 날짜 오류는 자동으로 값을 바꾸지 않는다. 충돌한 두 시점, 왜 저장할 수 없는지, 어느 입력을
  고쳐야 하는지를 필드 근처에서 말한다.
- 마일스톤은 이름·일정·안내를 가진 단계다. 새 마일스톤에는 제출 항목을 하나 기본으로 만들며,
  상위와 제출 항목 어디에도 `FILE/TEXT` 선택을 노출하지 않는다. 여러 자료를 따로 관리해야 할
  때만 제출 항목을 추가하고 선택적 참고 자료·양식을 붙인다.
- 저장된 마일스톤의 제출 항목 순서는 각 항목 앞의 드래그 손잡이로 바꾼다. 마우스·터치뿐 아니라
  키보드로도 손잡이를 집고, 방향키로 옮기고, 놓거나 취소할 수 있어야 한다. 이동 중 현재 위치와
  놓인 위치는 화면 읽기 도구에 한국어로 알리고, 동작 감소 설정에서는 위치 전환 애니메이션을
  끈다. 드롭이 끝났을 때만 전체 순서를 한 번 저장하며, 실패하면 서버가 마지막으로 확인한
  순서를 유지하고 해당 항목에서 다시 시도할 행동을 안내한다.
- 학생은 한 제출 화면에서 내용만, 파일만, 내용과 파일을 함께 낼 수 있다. 두 값이 모두 비었을
  때만 제출을 막고, `내용이나 파일을 하나 이상 추가해 주세요.`와 `둘 다 추가해도 됩니다.`를
  서로 다른 문장으로 안내한다.
- 생성과 편집은 같은 용어와 정보 순서를 쓴다. 저장 행동은 `임시 저장`과 `저장하고 계속`처럼
  결과가 명시적이어야 하며, 자동 저장처럼 보이게 만들지 않는다.
...
상태 설명만으로 끝내지 않는다. 화면에 들어온 사용자가 원래 하려던 일을 이어갈 수 있도록
`다시 시도`, `목록으로 이동`, `일정으로 이동`, `제출 항목 추가`처럼 목적에 맞는 다음 행동을
하나 이상 제공한다. 권한이 없으면 필요한 권한과 대신 갈 수 있는 화면을 함께 말한다.
...
| 관리자 | `운영 대시보드` | `/dashboard` (본문: 운영 대시보드. 관리 도구는 `/admin/*`) |
