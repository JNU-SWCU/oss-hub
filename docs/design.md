...
- 마일스톤은 이름·일정·안내를 가진 단계다.
  새 마일스톤에는 제출 항목을 하나 기본으로 만들며, 상위와 제출 항목 어디에도 `FILE/TEXT` 선택을 노출하지 않는다.
  여러 자료를 따로 관리해야 할 때만 제출 항목을 추가하고 선택적 참고 자료·양식을 붙인다.
- 저장된 마일스톤의 제출 항목 순서는 각 항목 앞의 드래그 손잡이로 바꾼다.
...
- 생성과 편집은 같은 용어와 정보 순서를 쓴다.
  저장 행동은 `임시 저장`과 `저장하고 계속`처럼 결과가 명시적이어야 하며, 자동 저장처럼 보이게 만들지 않는다.
