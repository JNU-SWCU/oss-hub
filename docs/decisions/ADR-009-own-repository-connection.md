...
> **2026-08-19 amendment — `OWN` 은 남고, 랭킹만 이 경로에서 떨어졌다.** 이 ADR이 확정한 것(read-only 연결, 가벼운 소유권 검증, 외부 활동을 프로그램 실적으로 셈, 제3자를 쿼리에서 좁힘)은 **전부 그대로 유효하다.** 다만 ADR-010의 2026-08-19 개정 노트로 **공개 랭킹이 external 편입에 더 이상 의존하지 않는다** — 랭킹은 사람 축(`GithubUserActivityHistory`)을 읽으므로 학생이 저장소를 하나도 연결하지 않아도 자기 한 해 활동이 집계된다. 그래서 `OWN` 연결의 의미는 약화되지 않고 **좁혀진다**: 이제 그것은 "이 저장소가 이 프로그램의 산출물"을 기록해 ② 프로그램 화면의 지표·타임라인·release를 만드는 경로이며, 그것이 `OWN`의 유일한 이유로 남는다. `제3자를 수집에서 자른다`는 §4의 경계는 저장소 축에서 그대로 지켜진다.
...
- 외부 기여자 수치(`전체 − 팀원합`)의 **저장 위치가 미정**이다. 현재는 구해서 로그로만 남긴다. 화면에 노출할 자리가 정해지면 그때 컬럼을 만든다 — 쓰이지 않는 컬럼을 추측으로 먼저 만들지 않는다.
  - 2026-09-26 해결: 자리는 교직원 팀 상세의 「팀원이 아닌 사람의 기여」 한 줄이다(#1133 후속). `GithubRepositoryOutsiderContribution`에 팀 저장소마다 프로그램 기간 안 commit·PR·issue 수만 담는다 — 아래 Changelog.
...
- 2026-09-26: 외부 기여자 수치의 저장 위치를 정했다(#1133 후속). 교직원 팀 상세의 「팀원이 아닌 사람의 기여 · Commit n · PR n · Issue n」 한 줄이 쓰며, `GithubRepositoryOutsiderContribution`이 팀 저장소마다 한 행을 수집할 때마다 덮어쓴다. commit은 이 ADR의 `외부 = 전체 − 팀원합`을 프로그램 기간으로 자른 값이다 — 전체는 `history(since:, until:) { totalCount }`(노드 없음, 1점), 팀원합은 같은 기간 지금 팀원의 commit fact 수. 그래서 commit은 봇이나 계정에 연결되지 않은 이메일의 commit을 가려내지 못한다. PR·issue는 목록을 이미 받는 경로라(「PR·릴리스는 적재 시 거른다」) 기간 시작 커서까지 읽어 작성자가 지금 팀원도 봇도 아닌 계정인 것만 메모리에서 센다. 사람 목록(교직원 화면의 옛 「웹 참여자와 연결되지 않음」)은 두지 않는다 — 비동의자 프로파일을 만들지 않는다는 2번 원칙 그대로다.
- 2026-08-19: 공개 랭킹이 사람 축으로 전환되면서 `OWN` 편입과 랭킹의 의존이 끊어졌음을 개정 노트로 덧붙였다(기존 본문 삭제 없음). `OWN` 연결 자체는 그대로 유지되며 ② 프로그램 화면의 지표·release 경로로 남는다. 근거는 ADR-010 2026-08-19 개정 노트다.
