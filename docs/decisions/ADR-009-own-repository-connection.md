...
> **2026-08-19 amendment — `OWN` 은 남고, 랭킹만 이 경로에서 떨어졌다.** 이 ADR이 확정한 것(read-only 연결, 가벼운 소유권 검증, 외부 활동을 프로그램 실적으로 셈, 제3자를 쿼리에서 좁힘)은 **전부 그대로 유효하다.** 다만 ADR-010의 2026-08-19 개정 노트로 **공개 랭킹이 external 편입에 더 이상 의존하지 않는다** — 랭킹은 사람 축(`GithubUserActivityHistory`)을 읽으므로 학생이 저장소를 하나도 연결하지 않아도 자기 한 해 활동이 집계된다. 그래서 `OWN` 연결의 의미는 좌하지 않고 **좁혀진다**: 이제 그것은 "이 저장소가 이 프로그램의 산출물"을 기록해 ② 프로그램 화면의 지표·타임라인·release를 만드는 경로이며, 그것이 `OWN`의 유일한 이유로 남는다. `제3자를 수집에서 자른다`는 §4의 경계는 저장소 축에서 그대로 지켜진다.
