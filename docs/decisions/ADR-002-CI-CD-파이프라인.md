...
- production 배포 트리거는 GitHub Actions(`deploy.yml`, `release: published`)가 비-draft·비-prerelease와 `DEPLOY_TRIGGER_ENABLED=true`를 확인한 뒤 HTTPS Jenkins 내장 원격 트리거(`buildWithParameters`, 빌드 토큰)로 보낸다. `created`·`published` 외 action과 full SemVer가 아닌 tag는 Jenkins가 거절한다. 트리거 엔드포인트·토큰 값은 GitHub repo secret에만 두고 저장소·로그에 남기지 않으며, nginx는 그 경로의 POST만 공개 프록시한다.
