...
**이 5종 구성은 관리자 요청에서 왔다** — release 개수 대신 star · 작업한 저장소 수 · issue 수로 세자는 요청이다. 제품·기획 결정의 원본은 [`AGENTS.md` Important Files](../../AGENTS.md#important-files)가 정한 대로 **Notion Decision Log**이며, 이 ADR은 그 결정을 여기서 다시 내리지 않고 기술적 귀결(테이블·획득 경로·축 경계)만 기록한다.
