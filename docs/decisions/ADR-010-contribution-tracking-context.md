...
- 2026-08-10: 신규 fact writer의 `githubId ∈ User` 경계를 ORG/EXTERNAL source-neutral로 통일해 팀 미특정 조직 저장소에서도 미가입자·작성자 불명 신원을 적재하지 않게 했다. 기존 레거시 행은 삭제하지 않고 별도 데이터 마이그레이션으로 추적하며, cutover parity도 같은 가입자 경계를 통과한 원장 수를 비교한다.
