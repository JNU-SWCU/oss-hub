...
high risk PR은 `MERGE_READY` 이후에도 PM인 @GoBeromsu와 Tech Lead인 @Lumiere001이 동일한 head·base에 대해 각각 manual accept를 남겨야 한다.
...
high risk는 이어서 `PM_ACCEPT head=<sha> base=<ref> base_sha=<sha>`를 @GoBeromsu가, `TECH_LEAD_ACCEPT head=<sha> base=<ref> base_sha=<sha>`를 @Lumiere001이 각각 남긴다.
