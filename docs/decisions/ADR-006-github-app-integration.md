...
`Collection App`은 조직 전체 repository의 metadata와 `push`·`release` webhook을 읽지만 repository를 생성하거나 collaborator·visibility를 변경할 수 없다.
...
`Collection App`은 Repository `Contents: read`와 `Metadata: read`만 요청한다.
...
1. Repository Operations App으로 합성 이름의 private repository 한 개를 생성하고 `201`과 external repository ID를 기록한다.
...
5. Collection App에서 실제 `push` 또는 `release` delivery 한 건의 valid signature 처리와 합성 invalid signature 거절을 확인한다.
6. 테스트 repository를 정리하고 조회 `404`로 삭제를 확인한다.
