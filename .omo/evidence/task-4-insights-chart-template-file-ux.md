...
- Authenticated integration driver (temporary spec deleted after run): `BACKEND_INTEGRATION_TEST_PATTERN='task-4-http\\.integration\\.spec\\.ts$' bash scripts/run-backend-integration.sh`.
...
- The harness applied migrations only to an isolated temporary PostgreSQL container, used synthetic rows, and its EXIT trap removed all containers/volumes. Cleanup receipt: `docker ps --format '{{.Names}}' | grep -E 'oss-hub-test-' || true` returned no output. Temporary integration spec was deleted and no process remains.
