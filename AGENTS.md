...
티켓 워크플로의 원본은 `skills/tickets/SKILL.md` 하나다 — QA 티켓 작성, GitHub Issue 발행, 티켓 수행을 모두 담으며 Codex 등 다른 에이전트도 티켓 요청을 받으면 이 파일을 따른다.
`.claude/skills/tickets`와 `.cursor/skills/tickets`는 이 원본을 가리키는 심볼릭 링크이므로 checkout만으로 Claude·Cursor가 스킬을 찾는다.
