...
const REQUIRED_PREFLIGHT_PATTERNS = [
  // 원장이 깨끗한가 — 끊긴 마이그레이션 위에서는 무엇을 되돌릴지조차 특정할 수 없다.
  ...
  // 배정된 legacy 관리자가 미분류로 남아 있으면 안 된다. bridge가 의도적으로
  // 비워 두는 유일한 부류이므로 그 이름으로 따로 센다.
  ...
  // 학번 전역 유일 — CHECK는 행 단위 술어라 유일성을 볼 수 없어 집합으로 센다.
  /GROUP BY "studentId" HAVING count\(\*\) > 1/,
  // 알 수 없는 요청 상태. 개명은 값을 옮기지 않으므로 그대로 정본 이름 밑으로 넘어간다.
...
const FORBIDDEN_SCHEMA_PATTERNS = [
  ...
  // 물리 이름을 정본 이름 위에 얹던 bridge 전용 이음매. 개명이 끝났으므로
...
/**
 * 정본 코드가 지워진 legacy 사실을 되살려 읽지 않는다는 것을 잠근다.
 *
 * 계약 이후 이 칸들은 물리적으로 없다 — 읽으면 런타임에 터진다. 타입 검사가 대부분을
 * 잡지만 raw SQL은 타입을 통과하지 않으므로 정적으로 따로 본다.
 */
const FORBIDDEN_SOURCE_PATTERNS = [
  ...
  // 개명이 끝났으므로 raw SQL은 정본 이름을 직접 쓴다. 물리 이름을 한 곳에 모아
  // 두던 bridge 전용 모듈은 사라졌고, 그 이름을 다시 쓰는 것은 회귀다.
  [
    /FROM\s+"RoleRequest"/,
