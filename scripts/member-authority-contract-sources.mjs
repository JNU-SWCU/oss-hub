...
/**
 * 계약(contract) 정적 검사가 **실제로 훑어야 하는 생산 소스**의 목록을 만든다.
 *
 * 검사기(`member-authority-contract-contract.mjs`)는 지워진 legacy 사실을 되살려
 * 읽는 코드를 잡는다. 그 검사가 의미를 가지려면 합성 문자열이 아니라 저장소에 실제로
 ...
 *
 * 픽스처·지원 모듈(`*.fixture.ts`, `*-support.ts` 등)은 **제외하지 않는다.** 걸리지
 * 않는 파일을 제외 목록에 올려 두면 나중에 그 파일이 진짜 위반을 담게 되어도 아무도
 * 모른다.
 *
 * 제외 목록이 조용히 넓어지지 않게 `member-authority-contract-contract.test.mjs`가
 * 이 정책을 따로 잠근다.
 */
