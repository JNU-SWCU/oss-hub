...
const baseEnvironment = Object.freeze({
  ...
  ROLLBACK_MINIO_ACCESS_KEY_ID: 'rollback-access-synthetic',
  ROLLBACK_MINIO_SECRET_ACCESS_KEY: 'rollback-secret-synthetic',
...
const syntheticSecrets = Object.freeze([
  ...
  baseEnvironment.ROLLBACK_MINIO_SECRET_ACCESS_KEY,
  sessionSecret,
  joinCodeSecret,
...
const controls = [
  ...
  control(
    'managed HTTPS R2 configuration accepts Jenkins-injected credentials',
    renderEnvironment({
      ...
      SUBMISSION_FILE_S3_FORCE_PATH_STYLE: 'true',
      SUBMISSION_FILE_S3_ACCESS_KEY_ID: REMOVE,
      SUBMISSION_FILE_S3_SECRET_ACCESS_KEY: REMOVE,
    }),
  ),
  control(
...
const valueRejects = [
  ...
  reject('unknown storage mode', 'SUBMISSION_FILE_STORAGE_MODE', 'r2'),
  reject(
    'minio mode rejects stale managed endpoint',
    'SUBMISSION_FILE_S3_ENDPOINT',
  ...
  removed('missing rollback MinIO bucket', 'ROLLBACK_MINIO_BUCKET'),
  removed('missing rollback MinIO access key', 'ROLLBACK_MINIO_ACCESS_KEY_ID'),
  removed(
    'missing rollback MinIO secret key',
    'ROLLBACK_MINIO_SECRET_ACCESS_KEY',
  ),
  reject(
    'rollback MinIO bucket must match application bucket',
    'ROLLBACK_MINIO_BUCKET',
    'different-synthetic-bucket',
  ),
  rawReject(
    ...
    'SUBMISSION_FILE_STORAGE_MODE',
    [baseEnvironment.SUBMISSION_FILE_S3_ACCESS_KEY_ID],
  ),
