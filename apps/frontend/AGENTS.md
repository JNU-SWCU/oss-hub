...
- **세션 상태 단일 소스**: 현재 사용자 조회는 `/auth/session`만 사용한다. 세션 정보가 필요하면 `useSessionRole`(공유 경로)을 경유한다. 로그인·로그아웃 직후의 화면 정합성은 공유 상태 또는 전체 리로드로 보장한다 — 컴포넌트별 로컬 state에만 반영하면 다른 세션 소비자가 stale 상태로 남는다.
