...
//
// 승인을 기다리는 교직원에게만 역할 없이도 연다(#581). 그들에게 되돌아갈 곳을 알려
// 주려던 `unassignedNotice`가 실제로는 되돌리는 처리에 가려 읽히지도 않았고, 오타 난
// 이름 하나를 고칠 방법이 없었다. 이제 안내는 화면 위에 남고 폼은 그대로 쓸 수 있다 —
// 역할이 필요한 것은 업무 화면이지 자기 프로필이 아니다.
//
// 누구에게 열지는 `isSettingsOpenForPendingStaff`가 정하고, 안내는 그 판단이 끝난 뒤
// 얹히는 표시다. 둘을 한 prop에 묶었다가 "안내를 준 화면"이 곧 "모든 미배정 사용자에게
...
export default function SettingsPage() {
  return (
    <RoleGate
      ...
      unassignedAccess={isSettingsOpenForPendingStaff}
