 ...
 *
 * 사유는 **관리자가 실제로 쓸 법한 문장**으로 둔다. 한 단어짜리로 두면 상자 폭·줄
 * 간격이 검토되지 않아 정작 실전에서 처음 깨진다.
 *
 * 여러 줄·긴 사유·이모지 경계·제어문자 같은 극단은 여기서 흉내내지 않는다. 그쪽은
 * `role-onboarding.test.tsx`의 `clampRejectionReason` 케이스가 값 단언으로 덮으므로,
 * 이 픽스처는 **검토자가 실제로 마주칠 화면**을 보여 주는 쪽에 집중한다 — 검토판이
 * 실전과 다르게 생기면 그 화면을 본 판단도 실전에 안 맞는다.
 */
