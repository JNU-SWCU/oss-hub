...
import { PROFILE_NAME_MAX_LENGTH, validateProfileForm } from './profile-state';
...
describe('profile onboarding view', () => {
  ...
  it('초기 폼에 16개 학과·트랙을 표시하고 기타 직접 입력은 숨긴다', () => {
    ...
    expect(html).not.toContain('기타(직접 입력)');
    expect(html).not.toContain('기타 학과');
    expect(html).toContain('GitHub 합성 이름');
  });

  it('잘못된 학번과 전화번호를 인라인 오류로 표시한다', () => {
    const html = renderForm(
      values({
        studentId: 'A'.repeat(6),
        phone: `${'1'.repeat(3)}-${'2'.repeat(4)}`,
      }),
  ...
  it('학생에게는 이름·학번·학과를 모두 보여 준다', () => {
    ...
    expect(html).not.toMatch(/id="profile-phone"[^>]*placeholder=/);
  });

  it('교직원에게는 학번과 전화번호 칸을 아예 보여 주지 않는다', () => {
  ...
  it('학과·학번·전화번호가 비어 있어도 가입 마치기 버튼은 눌린다', () => {
    ...
    expect(html).not.toContain('전화번호는 숫자 10~11자리로 입력해 주세요.');
  });

  it('빈 채로 제출하면 비어 있는 학번·전화번호·학과를 문구로 알려 준다', () => {
  ...
  it('오류 문구를 입력칸에 aria-describedby로 연결한다', () => {
    ...
    expect(html).toContain(
      'aria-describedby="profile-phone-description profile-phone-error"',
    );
    expect(html).toContain('id="profile-phone-error"');
