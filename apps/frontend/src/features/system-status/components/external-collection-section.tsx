 ...
 * 학생 GitHub 로그인 단위로 한 명씩 실행해야 한다. 이 사실을 빈 상태 문구에서
 * 정확히 설명한다 — "0을 그냥 0으로 보여주면 쓸모가 없다"는 이 화면의 원칙.
 */
const EXTERNAL_EMPTY_TITLE = '탐색된 학생 개인 GitHub 저장소가 아직 없습니다';
const EXTERNAL_EMPTY_DESCRIPTION =
  '학생 개인 공개 GitHub 저장소를 읽어 오는 수집 파이프라인은 조직 수집과 함께 매시 정각 자동으로 실행되고 있습니다. 다만 어떤 저장소를 수집할지 찾아내는 저장소 탐색은 자동으로 돌지 않고, 관리자가 학생별로 GitHub 계정을 한 명씩 지정해 실행해야 합니다. 지금까지 탐색을 실행한 학생이 없어 수집 대상이 0개이고, 그래서 매시 수집도 처리할 저장소 없이 끝나고 있습니다. 학생별로 저장소 탐색을 실행하면 다음 수집 주기부터 값이 채워집니다.';
