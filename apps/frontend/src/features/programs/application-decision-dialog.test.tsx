...
describe('ApplicationDecisionDialog — 키보드로도 빠져나올 수 있다', () => {
  ...
  describe('반려 확인창은 그 반려가 학생에게 무엇을 뜻하는지 말한다', () => {
    ...
    // 유일한 복구는 교직원의 「검토 대기로」다. 둘 다 확정 버튼을 누르기 전에 있어야 한다.
    function consequence(): HTMLElement | null {
      return document.getElementById('application-decision-reject-consequence');
    }
    ...
    it('학생이 스스로 다시 신청할 수 없다는 사실을 창 안에서 말한다', async () => {
      ...
      expect(notice?.textContent?.replaceAll(/\s+/gu, ' ').trim()).toBe(
        '반려하면 신청자는 이 프로그램에 스스로 다시 신청할 수 없습니다. 되돌리려면 이 화면의 「검토 대기로」를 눌러야 합니다.',
      );
    });

    it('되돌리는 길이 「검토 대기로」라는 것까지 같은 자리에서 말한다', async () => {
      // ⚠ 첫 문장만 남으면 교직원은 「잘못 눌렀으면 끝」으로 읽는다 — 사실이 아니다.
      //   복구 경로가 빠지지 않도록 따로 고정한다.
      await act(async () => root.render(<Harness action="REJECT" />));

      expect(consequence()?.textContent).toContain('검토 대기로');
    });

    it('겁주지 않는다 — 「영구히」·「되돌릴 수 없습니다」로 쓰지 않는다', async () => {
      // 되돌릴 수 있으므로 그렇게 쓰면 거짓이다(REVERT 는 반려를 언제나 푼다).
      await act(async () => root.render(<Harness action="REJECT" />));
    ...
    it('반려 창의 접근성 이름·설명 연결은 그대로다', async () => {
      // ⚠ 이 안내는 **보이는** 블록일 뿐 `AlertDialog.Description` 이 아니다 —
      //   설명 키를 넘기지 않는 기존 결정을 뒤집지 않는다.
      await act(async () => root.render(<Harness action="REJECT" />));

      const opened = dialog();
      expect(opened?.getAttribute('aria-describedby')).toBeNull();
