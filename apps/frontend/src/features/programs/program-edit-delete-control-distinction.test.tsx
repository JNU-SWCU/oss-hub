 ...
 * 위험 영역의 「프로그램 영구 삭제」와 헷갈리지 않는다. 그런데 그 상태를 지키는
 * 장치가 없었다 — 누군가 "아이콘만 있으니 무슨 버튼인지 모르겠다"며 마일스톤 줄에
 * 그냥 「삭제」를 붙이면, 교직원 화면에 「삭제」와 「프로그램 영구 삭제」가 나란히
 * 서고 어느 쪽이 프로그램을 통째로 날리는 버튼인지 이름만으로는 알 수 없게 된다.
 * 눈으로 보는 사람보다 화면 낭독기를 쓰는 사람이 먼저 다친다 — 낭독기는 버튼 목록을
 * 이름으로만 읽어 주기 때문이다.
 *
