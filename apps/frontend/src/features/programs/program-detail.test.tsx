...
describe('MilestoneRow', () => {
  ...
  /**
   * 반려된 신청(#1206). 옛 화면은 이 줄에서만 「신청 승인 후 제출 상태를 확인할 수
   * 있습니다」로 빠져나갔는데, 그 말은 반려 학생에게 거짓이다 — 기다린다고 승인되지
   * 않고, 교직원이 「검토 대기로」를 눌러 주어야만 열린다.
   *
   * 변이 검증 대상 — 반려를 승인과 같은 열린 상태로 취급하면 제출 버튼이 되살아나고,
   * 승인 대기와 한 문구로 묶으면 오지 않을 승인을 기다리게 하는 옛 문구가 돌아온다.
   */
  it('반려된 신청에는 반려를 이유로 적고 제출 경로를 세우지 않는다', () => {
    const html = renderToStaticMarkup(
      <MilestoneRow
        programId="program-1"
        position={1}
    ...
    expect(html).not.toContain('신청 승인 후 제출 상태를 확인할 수 있습니다');
    expect(html).not.toContain('제출 전');
    expect(html).not.toContain('제출하기');
    // 갈 수 없는 길은 내밀지 않는다 — 반려된 신청서는 수정도 취소도 되지 않는다.
    expect(html).not.toContain('다시 신청');
  });
