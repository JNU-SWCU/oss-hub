...
export function AuditLogView(props: AuditLogViewProps) {
  ...
  return (
    <section className="flex flex-col gap-6 p-4 sm:p-6">
      <PageHeader
        title="감사 로그"
        description="역할 요청 변경 이력을 행위자, 액션, 기간으로 조회합니다."
      />
      <form
        className="grid gap-4 rounded-lg border border-border p-4 sm:grid-cols-2 xl:grid-cols-5 xl:items-end"
        onSubmit={submit}
      >
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            id="audit-action"
            className="border-input bg-background focus-visible:border-ring focus-visible:ring-ring/50 min-h-11 rounded-md border px-3 text-sm outline-none focus-visible:ring-[3px]"
