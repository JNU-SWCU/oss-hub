...
9. 배포 서버가 실제로 쓰는 seed/운영 env는 **운영 credentials vault**에 codeblock으로, placeholder 없이 바로 실행 가능한 실값 그대로(키=값 줄) 보관한다. repo에는 루트 `.env.example`에 키 이름만 추가한다 — 일반 규칙은 [루트 AGENTS.md §2](../../../AGENTS.md)와 [보안 규칙](../../../docs/rules/security.md)이 원본이다.
