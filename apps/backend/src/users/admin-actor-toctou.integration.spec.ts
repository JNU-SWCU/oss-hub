 ...
 *
 * 네 가지를 각각 다른 방식으로 묻는다.
 * 1. 권한을 잃는 트랜잭션과 겹친 권한 변경이 **거부되는가** — 결과로 확인한다.
 * 2. actor를 읽은 뒤에 actor 행이 **정말 잠겨 있는가**(권한 변경 경로).
 * 3. 같은 것을 관리자 프로필 대리 수정 경로에서도.
 * 4. 강등이 **이긴** 경우의 프로필 대리 수정도 거부되는가 — 이 경로는 `RepeatableRead`라
 *    잠금 대기 중 강등이 커밋되면 P2034로 되돌아간다. `withSerializationRetry`가 새
 *    스냅샷으로 다시 돌려 결국 `ROL_004`가 나오는지를 실 DB에서 확인한다(#811 도입).
 *
...
/**
 * `admin-access-actor-toctou.integration.spec.ts`(#800)가 같은 이슈 번호에서 뽑은
 * `9_006_870_000n`을 이미 쓴다. `id`는 접두사로 갈리지만 `githubId`는 전역 유일이라
 * 두 스펙이 같은 DB에서 도는 순간 뒤에 도는 쪽이 unique 위반으로 죽는다 — 대역을 뗀다.
 */
...
it(
  ...
  async () => {
    ...
    // Then — 대기하던 잠금이 P2034로 깨지고, 재시도가 새 스냅샷에서 강등된 actor를 보고
    // 거부한다. 재시도가 없으면 여기서 원시 Prisma 에러가 500으로 새어 나간다.
    await expect(mutation).rejects.toMatchObject({
      errorCode: { code: RolesErrorCode.ADMIN_ONLY, status: 403 },
    });
    await expect(
