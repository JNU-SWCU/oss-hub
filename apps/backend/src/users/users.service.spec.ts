...
function buildService(
  ...
  const completeProfileIfIncomplete = jest
    .fn()
...
it('빈 프로필을 한 번만 저장하고 완료 응답을 반환한다', async () => {
  const { service, completeProfileIfIncomplete } = buildService();

  await expect(service.completeMyProfile(githubId, input)).resolves.toEqual({
    ...input,
    isComplete: true,
  });
  expect(completeProfileIfIncomplete).toHaveBeenCalledWith(
    'synthetic-user',
    input,
  );
});
...
it('이미 완료된 프로필은 수정하지 않고 409 사용자 오류로 거부한다', async () => {
  ...
  expect(completeProfileIfIncomplete).not.toHaveBeenCalled();
});
