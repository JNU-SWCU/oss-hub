...
async function cleanup(): Promise<void> {
  await prisma.notification.deleteMany({
    where: { userId: { in: [STAFF_ON, STAFF_OFF, ADMIN_ON, STUDENT] } },
  });
  ...
  await prisma.user.deleteMany({
    where: { id: { in: [STAFF_ON, STAFF_OFF, ADMIN_ON, STUDENT] } },
  });
}
...
it('알림 켠 교직원·ADMIN에게만 발송하고 SENT를 기록한다(끈 교직원·학생 제외)', async () => {
