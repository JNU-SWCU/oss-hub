 ...
 * REPOSITORY_RELEASE는 `{ type, releaseUrl }`, FILE은 `Prisma.JsonNull`을 쓴다. 여기서는 그
 * 모양을 그대로 되꺼낼 뿐 새 규칙을 만들지 않는다.
 *
 ...
 * 2. 한 칸의 크기는 이미 유계다. 제출 요청이 `text`를 10,000자, `releaseUrl`을 2,048자로
 *    막는다(`create-milestone-document-submission-request.dto.ts`). 상한을 다시 두어도
...
export function readMilestoneDocumentSubmittedContent(
  ...
  switch (content.type) {
    ...
    case MilestoneSubmissionType.REPOSITORY_RELEASE:
      return typeof content.releaseUrl === 'string'
        ? {
