...
describe('authentication route metadata manifest', () => {
  ...
  it('classifies every discovered route and locks the complete inventory', async () => {
    ...
    expect(routesByAccess[AUTH_ROUTE_ACCESS.PUBLIC]).toHaveLength(12);
    ...
    expect(routesByAccess[AUTH_ROUTE_ACCESS.PROTECTED]).toHaveLength(108);
    expect(manifest).toHaveLength(124);
    expect(
      manifest.filter(
        ({ method, path }) =>
