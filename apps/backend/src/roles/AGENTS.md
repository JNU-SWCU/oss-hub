...
# apps/backend/src/roles — 역할 온보딩·교직원 승인·관리자 사용자 관리

## Purpose

역할 선택, 교직원 승인 요청, 관리자의 역할 강제 변경 흐름을 담는다.
...
| `admin-users.service.ts` | ADMIN의 사용자 역할 강제 변경(`PATCH`) — 마찬가지로 audit-log 기록 |
...
| 경로 | 내용 |
| --- | --- |
| `domain/` | `role-onboarding.ts`(선택 가능 역할·결과 타입) · `admin-user.ts` · `staff-role-request.ts` |
| `dto/` | 역할 선택·교직원 요청·관리자 사용자 조회 요청/응답 DTO |

## For AI Agents

- 기존 `STUDENT`가 다시 학생을 선택하는 경로는 멱등 성공한다.
- 다른 역할이 이미 있거나 역할 보유자가 STAFF를 선택하면 `ROLE_ALREADY_CONFIRMED`를 던진다.
- 확정된 역할 변경은 ADMIN의 `admin-users.service.ts` 경로로만 수행한다.
...
- 교직원 요청·관리자 사용자 변경 두 서비스 모두 `AuditLogService.record`를 같은 트랜잭션의 `store.auditLogWriter`로 넘겨 커밋 원자성을 보장한다 — 감사 로그를 별도 트랜잭션으로 쓰지 않는다.
