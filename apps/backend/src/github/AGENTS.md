<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-07-20 · Updated: 2026-08-10 (system-status ADMIN 화면용 `getIncrementalStatusStreams` 추가 — 저장소 이름을 노출하지 않는다는 집계 경계를 그 상세 메서드에 한해 옮긴 owner 결정; 이어서 ADR-003 DEC-42 boundary-lint 수정으로 `getNextScheduledCycleAt` 추가 — cron 표현식 계산을 github 모듈 안으로 옮겼다) -->
