...
| `collection-read.port.ts` | 삭제됐다. `COLLECTION_READ_PORT`는 없다. 같은 DB 조회는 소비자 모듈의 query repository가 소유한다 — [ADR-003](../../../../docs/decisions/ADR-003-backend-architecture.md). |
