<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-07-20 · Updated: 2026-08-20 (`COLLECTION_READ_PORT` 삭제 — 같은 DB 조회는 소비자 repository, ADR-003) -->
...
| `collection-read.port.ts` | 삭제됐다. `COLLECTION_READ_PORT`는 없다. 같은 DB 조회는 소비자 모듈의 query repository가 소유한다. 다른 모듈 Service를 조회 버스로 쓰는 hop도 같은 안티패턴이다 — [ADR-003](../../../../docs/decisions/ADR-003-backend-architecture.md). |
