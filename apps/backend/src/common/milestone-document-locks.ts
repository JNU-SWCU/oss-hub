/**
 * 마일스톤·서류 항목 행 잠금 원시(raw SQL 행 잠금) — 업무 규칙이 아니라 **공용 영속성 도구**다.
 *
 * `milestone-documents`(서류 항목 추가·삭제·순서 재부여)와 `programs`(마일스톤 삭제)가 **같은
 * 행을 같은 순서로** 잠가야 직렬화된다. 어느 한쪽 모듈이 이 문장을 소유하면 다른 쪽이 형제
 * 모듈의 영속성 동작을 계약 없이 실행하게 되고(ADR-003의 cross-module behavioral dependency),
 * 소유자 쪽 잠금 전략이 바뀌면 남의 경로가 조용히 깨진다. 그래서 두 모듈이 함께 쓰는 기반
 * 계층인 `common/`에 두어 소유자를 없앤다. Port를 새로 만들지 않는 것은 ADR-003의 「기능
 * 요구가 없는 포트·어댑터·추가 추상화는 도입하지 않는다」를 따르기 때문이다 — 옮기는 쪽이 더
 * 작고 정확하다.
 *
 * **전역 잠금 순서 규칙은 이 파일 하나에 모여 있다** — 아래 `lockMilestoneDocumentsOfMilestone`의
